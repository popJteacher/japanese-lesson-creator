...
**`prompts/master_prompt_design_guide_v3_4.py`**（LF 正規化後 hash 先頭：`738af3e76983`）。
...
- v3.4 で残る backlog: M-15 wave（D/H/J を `{STRATEGY_BLOCK}` 構造化）→ apply_v3_5.py で対応予定
- v3.3 主要変更：
...
| `prompts/master_prompt_design_guide_v3_4.py` | `738af3e76983` | **LF 正規化後**（`invariants.mjs` B の比較方法。canonical 正典） |
...
| `apply_v3_4.py` | — | `archive/prompts/` 配下（63 patch・件数アサート付き / v3.4 確定後は検証時のみ再実行） |
...
| プロンプトガイド | `prompts/master_prompt_design_guide_v3_4.py`（v3.2/v3.3 は archive 候補） |
