...
| `vocabCount` | ✅ | 数 |
| `examples` | ✅ | 配列 |
| `applicationExamples` | 任意 | 配列（空配列可）。応用・応答例 |
| `practiceTemplates` | ✅ | 配列 |
| `shareVocabWith` | 任意 | 別文型 ID |

#### `patterns[].examples[]` / `patterns[].applicationExamples[]` の役割分担

**最重要ルール（lesson-scaffold / lesson-check skill で強制）：**

- **`examples[]`** = 各課 PDF の **文型欄（一番始めに提示される基本形例文）** に明記されたものだけを置く。
  Q 形式 or 基本宣言形のみ。応答・応答セット（「— はい、〜です」等）や応用（省略形・複合場面・活動例）は含めない。
  **宿題 generator (`homework_html.js`) はこの配列だけを参照する**ため、controlled practice の controlled 性を担保する。
  practiceTemplates の blank 構成と整合（uniform 推奨）。
- **`applicationExamples[]`** = 「教え方の例」「プラスα」「活動例」「会話例」など、PDF 文型欄**以外**から拾った応用・応答表現を置く。
  sentence 形は出典のまま（A 単独 / Q+A 結合 / 複合会話 / 応答セット）。一律 A 単独に揃えない。
  **画像不要**（`imageId` 省略可・registry に entry を作らない）。
  **宿題練習問題には使われない**。render 先（会話例スライド / 教案 docx 応用ガイド）は backlog 段階。
- 両者は同じ pattern 配下にあるが、データソース・render パス・教育的役割が異なる。混在させない。
...
#### `patterns[].applicationExamples[]`（v2.13+）

| フィールド | 必須 | 用途 |
|---|---|---|
| `no` | ✅ | `app-N` 形式（例: `app-1`, `app-2`）— `examples[].no` と区別 |
| `sentence` | ✅ | 日本語（出典のまま。A 単独 / Q+A 結合 / 複合会話 可） |
| `sentenceEn` | ✅ | 英訳 |
| `imageId` | 任意 | 省略可。原則 applicationExamples は画像なし |
| `isAnchor` | 任意 | 常に `false`（省略可） |
| `originalSource` | 任意 | 出典 PDF page / textbookRef（例: `"PDF p.031 教え方の例"`） |
| `_comment` | 任意 | メモ |
...
| `_sourceTag`（出どころ enum・Phase 1 of Goi_List 統合 / 2026-05-28+） | ✅ |

#### `_sourceTag` enum（`lesson-check` B-15 / B-16 で強制）

| 値 | 意味 | 検証 |
|---|---|---|
| `pdf_introduction` | 各課 PDF の「導入語彙」リストに明記された語 | 人間 review（PDF 照合は機械化不可） |
| `goi_list_n5_supplement` | `data/sources/goi_list_raw.json` の N5 範囲から能動推薦で追加 | B-16 が Goi_List N5 範囲に実在することを ERROR で検証（捏造防止） |
| `teacher_addition` | PDF にも Goi_List N5 にもないが教師判断で追加（例：副詞・接続表現の補強）| 人間 review |
| `inherited_from_earlier_lesson` | 過去課で既出。本課は復習として再掲 | `_inheritedFromLessonX` フィールドと整合する想定 |
| `practice_only` | 例文には登場せず practice template の素材だけに使う / 意図的に sentence 不在 | B-14（vocab vs sentence 整合性 WARN）が skip される |

- **Phase 2 着地（2026-05-28+）**：`lesson-scaffold` の Step 4.5「Goi_List N5 補強候補対話フェーズ」が実装済。`scripts/lib/lesson-goi-supplement.mjs --no NN --candidates` で対象課以前の全 lesson_*.json と dedup した N5 候補プールを抽出し、Claude が pattern コンテキストに沿って curate → user と y/n/skip 対話 → `--adopt path --apply` で書き戻す。helper が `_sourceTag="goi_list_n5_supplement"` を自動付与し B-16 相当の N5 実在検査を同時に行う。skill 外でも standalone で呼べる（既存 lesson への追加補強用）。
- 関連 lint: **B-14**（vocab word が examples/applicationExamples/practiceTemplates に登場するか WARN・犬/写真 問題の再発防止）/ **B-15**（_sourceTag 必須・enum 内 ERROR）/ **B-16**（goi_list_n5_supplement の Goi_List N5 実在 ERROR）。
