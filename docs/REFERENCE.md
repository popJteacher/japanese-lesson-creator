...
**`prompts/master_prompt_design_guide_v3_3.py`**（LF 正規化後 hash 先頭：`80de1a8e675f`）。

- v3.2 → v3.3 適用器：`archive/prompts/apply_v3_3.py`（件数アサート付き決定論パッチ・79 編集 / 72 件監査一括是正）
...
| `prompts/master_prompt_design_guide_v3_3.py` | `80de1a8e675f` | **LF 正規化後**（`invariants.mjs` B の比較方法。Windows で `apply_v3_3.py` を再実行すると CRLF 出力になるため、raw 比較は偽陽性。正規化後 SHA で照合する） |
| `prompts/master_prompt_design_guide_v3_2.py` | `566b8ad68753` | **LF 正規化後**（旧正典。v3.3 確定後は archive 候補・参照のみ） |
| `apply_v3_3.py` | — | `archive/prompts/` 配下（79 patch・件数アサート付き / v3.3 確定後は検証時のみ再実行） |
...
| プロンプトガイド | `prompts/master_prompt_design_guide_v3_3.py`（v3.2 は archive 候補） |
