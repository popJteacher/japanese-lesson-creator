...
| 項目 | 値 | 出所 |
|---|---|---|
| SPREADSHEET_ID | `1l-gH5MPfRyqNyuzj9x3c2xQoQeji60yF7mKc7_50Nlk` | line 24 / 3066 |
| IMAGE_REGISTRY_ID | `17WnltHEvymkua4hgfak2951f5BgphV9O` | line 3067（2026-05-19 新 ID） |
| AUDIO_REGISTRY_ID | `1y0-mzxQGfZVHyj6tT1ttXzt0knlueb3M` | line 3068（2026-05-19 新 ID） |
...
| `prompts/master_prompt_design_guide_v3_2.py` | `566b8ad68753` | **LF 正規化後**（`invariants.mjs` B の比較方法。raw は `5d7e52f00e3f` だが Windows で `apply_v3_2.py` を再実行すると CRLF 出力になるため、raw 比較は偽陽性になる。正規化後 SHA で照合する） |
| `apply_v3_2.py` | — | `archive/prompts/` 配下に保管（v3.2 確定後は再実行不要・検証時のみ） |
