...
## 2026-05-27 (β1-v0.3 multi-template + applicationExamples plan 確定)

- **β1-v0.3 multi-template render 着地**: homework_html.js `practiceTemplates[0]` 固定 → 全 templates 縦並びに展開。uniform 構成 (全 templates 同 blank 数) のみ judge UI 有効、異形は自己採点に降格。blank=0 templates は `.template-row.no-input` で灰色テキスト hint 表示。これで lesson_02 p2「これは何ですか」3 件 / p6「どの人ですか」の「judge-btn 出るのに input 0」矛盾 bug が解消。lesson_01 退行: p1/p3 judge UI 維持、p2 templates 異形のため judge UI 消失 (設計判断)。lesson_02 p1/p5 で uniform 多 template の judge UI 動作確認 (vocab 14 件 × 3 templates = 42 judge OK / NAMED 5 件 × 5 templates = 15 judge OK)。
- **applicationExamples schema plan 確定 (実装は別 session)**: user 確定の基本ルール「PDF 文型欄 (各課の一番始めに提示される基本例文) = `examples` / 教え方の例・プラスα・活動例 = `applicationExamples`」を skill (lesson-check / lesson-scaffold) で強制する方針。sentence 形は出典のまま (A 単独 / Q+A 結合 / 複合会話 を case-by-case 許容)、applicationExamples は **画像不要** (registry に entry なし、`section` field 追加も不要)。既存 Q+A 場面画像は examples sentence (Q 形式に整理) に流用、Q 単体最適化画像への再生成は backlog。applicationExamples の render 先 (会話例スライド or 教案 docx の応用ガイド section) は backlog。教育学的根拠: controlled practice (ドリル = 「練習しよう！」スライド) と communicative practice (応用会話 = applicationExamples) は別ステップとして提示すべきで混在させない。
- **lesson_02 Drop&Drag (plan に含む)**: intro_act_p5 / intro_act_p6 の materialNeeds[] に `{type: 'teacher_photo', count: 4}` を追加することで `renderMaterialDrivenLayout` fallback path が有名人写真スロット × 4 を自動描画 ([slide_html.js:2093](src/generators/slide_html.js#L2093) 既実装)。act_famous_person_intro 本体は変更不要。
- **main_act suggest 拡張**: `act_whiteboard_categorize.validatedForLessons` に `2` を追加 (1 行修正) で `/lesson-suggest-activities 02` の候補に並ぶ。
