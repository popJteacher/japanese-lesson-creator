...
// ===== Range A: generateImages.gs main block + 23 functions =====
// 元配置: gas/pipeline.gs (v7.4) line 750-2528 (cut 2026-05-21 Phase 4 ⑥)
// =================================================================
...
function generateImageBatch() {
  ...
  Logger.log("===== バッチ完了 =====");
  Logger.log("成功: " + successCount + " / エラー: " + errorCount + " / スキップ: " + skipCount + " 件");
  if (remaining > 0) {
...
function processImageEntry_(ss, row) {
  // [Change 1] S列（imagePrompt）が空または50文字以下なら生成スキップ
  if (!row.imagePrompt || row.imagePrompt.trim().length <= 50) {
    const sheet = ss.getSheetByName(IMAGE_SETTINGS.VOCAB_SHEET_NAME);
    if (sheet) sheet.getRange(row.rowIndex, 8).setValue("skipped_no_prompt");
    writeImageLog_(ss, row.imageId, "skipped", "", "S列プロンプトなし（50文字以下）");
    Logger.log("  ⏭️  スキップ: " + row.imageId + " → S列プロンプトなし（50文字以下）");
    return "skipped";
  }
  ...
  try {
    ...
    // [Change 3] リトライ上限管理：現在のステータスから失敗回数を読み取り、failed_N / failed_final に更新
    const sheet = ss.getSheetByName(IMAGE_SETTINGS.VOCAB_SHEET_NAME);
    let currentFailCount = 0;
    if (sheet) {
      const cur = String(sheet.getRange(row.rowIndex, 8).getValue() || "").trim();
      const m = /^failed_(\d+)$/.exec(cur);
      if (m) currentFailCount = parseInt(m[1], 10);
    }
    const newCount  = currentFailCount + 1;
    const newStatus = newCount < 3 ? ("failed_" + newCount) : "failed_final";

    if (sheet) sheet.getRange(row.rowIndex, 8).setValue(newStatus);
    writeImageLog_(ss, row.imageId, newStatus, "", e.message);
    Logger.log("  ❌ " + newStatus + ": " + row.imageId + " → " + e.message);
    return "error";
  }
}
...
function getPendingImageRows_(ss) {
  // [Change 2] pending と failed_1/2/3 を再試行対象に含める
  // skipped_no_prompt と failed_final は除外する
  // pending 行を優先処理し、その後に failed_* 行を追加する（API枠を pending に優先確保）
  const all     = getAllImageRows_(ss).filter(function(r) { return r.vocabType !== ""; });
  const pending = all.filter(function(r) { return r.imageStatus === "pending"; });
  const failed  = all.filter(function(r) { return /^failed_[123]$/.test(r.imageStatus); });
...
// ===== Range B: loadJsonFromDriveById + 9 Sheet 操作 utility =====
// 元配置: gas/pipeline.gs (v7.4) line 2545-2803 (cut 2026-05-21 Phase 4 ⑥)
// 退役理由: Phase 2 registry SSOT 化後、Sheet 直接操作は anti-pattern。
//          loadJsonFromDriveById は importExamplesFromLesson02 の唯一
//          consumer が退役するため同時引退。
// =================================================================
...
function resetFailedToPending(lessonRef) {
  ...
  for (let i = 0; i < data.length; i++) {
    ...
    // failed / failed_1 / failed_2 / failed_3 / failed_final など全 failed_* を対象に
    if (!/^failed/.test(imageStatus)) continue;
