# 引き継ぎ資料 v4.4
**バージョン：v4.4（2026-05-18）**
**前資料：handoff_v4_3.md（2026-05-18）**
**このバージョンで追加した内容：音声生成パイプライン専用引き継ぎ（`audio_pipeline_handoff_v2_0.md` v2.0）を全面統合。WAVヘッダー二重化バグの根本原因特定と Phase 1 修正完了を記録。音声後処理 Phase 2〜4（音量正規化・無音調整・SSML）を未対応として記録。v4.3 §13 の「buildWavBlob_() 等の変更不要」記述の誤りを訂正。「Phase」という語の名前空間衝突（音声後処理 Phase / TTSプロバイダ移行 / プロジェクト設計フェーズ）を整理。**
...
| **③** | **CLAUDE.md 更新** | L147: lesson_01→v1.1・L148: lesson_02→v2.11.11・L149: SKILL_v1_6.md・generateAudio.gs **v3.0 + Phase 1 WAVヘッダー修正適用済み** を追記（§3 の注記・§14 参照） |
| **④** | **lesson_03.json 作成開始** | `SKILL_v1_6.md` の Step 1a（骨格設計）から開始 |

> **次チャットへのアップロード必須ファイル：**
> - `handoff_v4_4.md`（この資料・`audio_pipeline_handoff_v2_0.md` を §14 に全面統合済みなので audio 専用資料の別途アップロードは不要）
...
> **⚠️ CLAUDE.md の要更新箇所（v4.3 で判明・v4.4 で追記・次チャットで対応）：**
...
> - **（v4.4 追加）generateAudio.gs / setupSpreadsheet.gs に「Phase 1：WAVヘッダー二重化バグ修正適用済み（`buildCloudTtsWavBlob_()` 新設・`callGoogleCloudTTS_()` 戻り値を `audioBase64` に変更・`validateWavStructure_()` / `resetToRegenerate()` 追加）」を追記。CLAUDE.md には音声パイプラインの記述が現状ほぼ無いため、最小でも「音声は Google Cloud TTS WaveNet・WAV(LINEAR16)・Phase 1修正済み・Phase 2〜4 未対応」の1行を追加すること**
