...
# Version 2.6 | 2026-05-18 (v2.9改善の選択的取り込み)
...
## v2.9 の CHARACTER_RENDERING_POLICY を選択的取り込み。
##
## 語彙カード（person）と例文画像（example_sentence）では肌色ルールが異なる。
## STYLE_RECIPE に肌色を混在させると誤適用の原因になるため分離。
## プロンプト生成時は vocab_type を確認し、下記の skin_prompt_phrase を
## CONSTRAINTS ブロックの後に追記すること。
## ============================================================
...
CHARACTER_RENDERING_POLICY = {
  ...
  "warm_skin_realistic": {
    ...
    "spec": "Naturally warm medium skin tone — avoid both extremely pale and extremely dark defaults.",
    "skin_prompt_phrase": "Skin tones: naturally warm medium skin tone.",
    "_note": "語彙カード（職業・役割・家族）の人物に適用。"
  },
  ...
  "critical_note": (
    "同一レッスン内で person 語彙カード（暖色肌）と example_sentence（グレーアバター）が"
    "混在しても問題ない。前者は職業の学習、後者は文型の学習で認知タスクが異なる。"
    "重要なのは『同じ vocab_type 内では一貫していること』。"
  )
}
