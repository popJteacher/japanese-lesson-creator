...
# Version 3.1 | 2026-05-19 (= v3.0 + 問題F完遂 + 背景クリーム化リグレッション是正)
...
STYLE_BIBLE = {
  ...
  "color_palette": {
    ...
    "note": "v3.0方針→v3.1で完遂: 色名のみ運用。#HEX や 'hex value' をプロンプトに一切書かない（v3.0は37箇所削除と記載も実際は本文/ARROW_SEMIOTICS/reset_promptに残存。v3.1で実削除し完了）。色名表記の一字一句を変えないことでスタイルドリフトを防止する（資料6）。"
  },
  ...
  "vocabulary_card_style":
    "Solid soft cream off-white background (warm off-white, not pure stark white). Single centered subject only. "
...
NATIONALITY_NOUN_POLICY = {
  ...
  "principle":
    ...
    "non-stereotyped appearance fills 70-80% of the frame. Nationality is "
  ...
  "flag_pin_rule":
    ...
    "patch on the chest or on a bag strap. The flag must occupy at most "
    "about 4% of the image area. It is a subtle secondary identifier, "
  ...
  "hard_constraints": [
    ...
    "DO NOT use stereotyped national costume, props, or caricatured "
    "features as the nationality cue (project policy).",
  ],
  ...
  "subject_block_pattern":
    ...
    "clear main subject and fills 70-80% of the frame.",
...
→ Render in a bold accent color (warm amber gold or bright red).
...
[STRATEGY: MOTION_ARROW]
  ...
  the path or direction of the action. The arrow color: warm amber gold or bright red.
...
ARROW_SEMIOTICS = {
  ...
  "straight_bold": {
    ...
    "color":   "Warm amber gold or bright red"
  },
