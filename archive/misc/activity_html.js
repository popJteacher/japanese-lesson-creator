...
function buildActivityHtml(session, lessonDataMap, imageRegistry, activityCatalog) {
  const selectedId = session.activity?.selectedId;
  if (!selectedId) return null;

  const actDef = (activityCatalog.activities || []).find(a => a.id === selectedId);
  if (!actDef || actDef.contentRequirement?.judgment !== 'required') return null;
...
rt{font-size:.52em;color:var(--text-subtle);font-weight:400;transition:opacity .25s var(--ease)}
...
function buildFullPage(session, actDef, mainContent) {
  ...
  const lessonLabel = lessonNos.map(n => `第${n}課`).join('・') || '—';
...
<head>
  ...
  <title>${h(lessonLabel)} アクティビティ｜${h(titleJa)}</title>
...
<body>
  ...
  <header class="page-header">
    <span class="lesson-badge">${h(lessonLabel)}</span>
    <h1>アクティビティ</h1>
  ...
  return `
    ...
    <div class="card mb2">
      <div class="status-bar">
        ...
        <div class="status-item"><span class="label">試行回数</span><span class="value" id="mm-tries">0</span></div>
...
function buildVocabBingo(lessons, taughtPids, actDef) {
  ...
  return `
    <div class="card mb2">
      <p class="section-heading">アクティビティ</p>
      <h2 class="activity-title">語彙ビンゴ</h2>
      <p class="activity-title-en en-text">Vocabulary Bingo</p>
      <div class="mt1">
        <p class="setting-text">
          ${withRuby('先生','せんせい')}のヒントを${withRuby('聞','き')}いて、ビンゴカードの中から${withRuby('合','あ')}う${withRuby('単語','たんご')}を${withRuby('選','えら')}んでマークしてください。
    ...
    <div class="card mb2">
      <div class="status-bar">
        <div class="status-item"><span class="label">マーク数</span><span class="value" id="bg-count">0</span></div>
        <div class="status-item"><span class="label">状態</span><span class="value" id="bg-status">プレイ中</span></div>
    ...
    <div class="card mb2">
      <p class="section-heading">先生パネル（${withRuby('呼','よ')}び${withRuby('出','だ')}し${withRuby('用','よう')}）</p>
      <p style="font-size:.8rem;color:var(--text-subtle);margin-bottom:.75rem">
        ここからヒントを選んで読み上げてください。学習者には見せないでください。
      </p>
  ...
  (function(){
    ...
    function setStatus(s){document.getElementById('bg-status').textContent=s;}
...
function buildWhiteboardCategorize(lessons, taughtPids, actDef) {
  ...
  return `
    <div class="card mb2">
      <p class="section-heading">アクティビティ</p>
      <h2 class="activity-title">語彙の仲間分け</h2>
    ...
    <div class="card mb2">
      <div class="status-bar">
        <div class="status-item"><span class="label">正解</span><span class="value" id="cat-correct">0 / ${allWords.length}</span></div>
        <button class="btn btn-ghost" onclick="catReset()" style="margin-left:auto">🔄 リセット</button>
        <button class="btn btn-accent" onclick="catCheck()">✓ 確認</button>
      </div>

      <!-- Word Pool -->
      <p class="section-heading mt1">単語プール</p>
  ...
  (function(){
    ...
    window.catCheck=function(){
      let ok=0;
      WORDS.forEach((w,i)=>{
        const el=document.querySelectorAll('.cat-word')[i];
        /* find in zone */
        const zone=document.querySelector('#zone-'+placement[i]);
        if(!zone)return;
        const elInZone=[...zone.querySelectorAll('.cat-word')].find(e=>e.textContent.includes(w.word.replace(/<[^>]+>/g,'')));
        if(elInZone){
          elInZone.classList.remove('correct-placed','wrong-placed');
          elInZone.classList.add(placement[i]===w.catId?'correct-placed':'wrong-placed');
          if(placement[i]===w.catId)ok++;
        }
      });
...
function buildGrammarAuction(lessons, taughtPids, actDef) {
  ...
  return `
    <div class="card mb2">
      <p class="section-heading">アクティビティ</p>
      <h2 class="activity-title">文法オークション</h2>
    ...
    <div class="card mb2">
      <div class="status-bar">
        <div class="status-item"><span class="label">残コイン</span><span class="value" id="auc-coins">${BUDGET} 💰</span></div>
        <div class="status-item"><span class="label">問題</span><span class="value" id="auc-progress">1 / ${sentences.length}</span></div>
      ...
      <div style="display:flex;gap:.75rem;align-items:center;flex-wrap:wrap;margin-bottom:1rem">
        <label style="font-size:.85rem;font-weight:700">賭けるコイン：</label>
        ...
        <button class="btn btn-accent" onclick="aucBid()">賭ける！</button>
  ...
  (function(){
    ...
    function resolve(bet){
      ...
      if(s.correct){
        ...
        res.innerHTML='<b style="color:var(--correct)">✓ 正文！</b> +'+(bet)+'コイン<br><small style="color:var(--text-subtle)">'+s.en+'</small>';
      } else {
      ...
      setTimeout(()=>{
        render();
        if(idx>=SENTS.length) showSummary();
      }, 1800);
    }
...
function buildBattleship(lessons, taughtPids, actDef) {
  ...
  return `
    <div class="card mb2">
      <p class="section-heading">アクティビティ</p>
      <h2 class="activity-title">戦艦ゲーム</h2>
      <p class="activity-title-en en-text">Battleship</p>
      <div class="mt1">
        <p class="setting-text">
          グリッドを使って相手の${withRuby('船','ふね')}を${withRuby('見','み')}つけてください。<br>
    ...
    <div class="card mb2">
      <p class="section-heading">マイボード — 船を置く（3マス）</p>
      <p style="font-size:.8rem;color:var(--text-subtle);margin-bottom:.5rem">クリックで自分の船を置いてください（最大3マス）</p>
      <div id="bs-my-board"></div>
    </div>

    <div class="card mb2">
      <p class="section-heading">攻撃ボード — ここで質問する</p>
    ...
    <div class="status-bar mt1">
      <div class="status-item"><span class="label">命中</span><span class="value" id="bs-hits">0</span></div>
      <div class="status-item"><span class="label">外れ</span><span class="value" id="bs-misses">0</span></div>
  ...
  <style>
    ...
    .bs-grid ruby rt{font-size:.48em;color:var(--text-subtle)}
  </style>
...
function buildPersonGuessingQuiz(lessons, taughtPids, actDef) {
  ...
  return `
    <div class="card mb2">
      <p class="section-heading">アクティビティ</p>
      <h2 class="activity-title">人物当てクイズ</h2>
    ...
    <div class="card mb2">
      <div class="status-bar">
        <div class="status-item"><span class="label">問題</span><span class="value" id="pq-progress">1 / ${chars.length}</span></div>
        <div class="status-item"><span class="label">正解</span><span class="value" id="pq-score">0</span></div>
      ...
      <!-- Question Buttons -->
      <p class="section-heading">質問して答えよう</p>
      ...
      <div style="display:flex;gap:.5rem;align-items:center;flex-wrap:wrap">
        <span style="font-size:.9rem;font-weight:700">この人は</span>
        ...
        <button class="btn btn-primary" onclick="pqGuess()">決定！</button>
      </div>
...
function buildPersonalityQuiz(lessons, taughtPids, actDef) {
  ...
  return `
    <div class="card mb2">
      <p class="section-heading">アクティビティ</p>
      <h2 class="activity-title">性格診断クイズ</h2>
    ...
    <div class="card mb2" id="pz-quiz-area">
      <div class="status-bar">
        <div class="status-item"><span class="label">質問</span><span class="value" id="pz-progress">1 / ${questions.length}</span></div>
    ...
    <div class="card mb2" id="pz-result-card" style="display:none">
      <p class="section-heading">診断結果</p>
      ...
      <div style="margin-top:1rem;background:var(--bg-subtle);border-radius:var(--radius-md);padding:1rem;text-align:center">
        <p style="font-size:.85rem;font-weight:700;margin-bottom:.5rem">自己紹介文を作ろう</p>
    ...
    <div class="card mb2">
      ...
      <div class="dialogue">
        ...
        <div class="dialogue-turn turn-a">
          ...
          <div class="bubble">
            <p class="bubble-ja">
              ${charB.name}は${withRuby(jobA,vocab.find(v=>v.word===jobA)?.reading||'')}ですか。
            </p>
            <p class="bubble-en en-text">
              Are you a ${jobA}, ${charB.name}?
            </p>
          </div>
        </div>
