...
**X-a-3 (suggest-activities + build-registry)** 実装済の挙動 🆕:

- `/lesson-suggest-activities NN [--type intro|main|all] [--top N] [--json]`：
  read-only。flow の intro_activity / main_activity の各 step に対し、jlptLevel /
  grammarConcept / fadingStage / prerequisitePatterns / validatedForLessons /
  textbookOrigins で scoring した候補を上位 N 件提示。★ は既設定 activityId
- `/lesson-build-registry NN [--apply] [--json]`：
  lesson_NN.json が参照する全 imageId / audioId を集計し、master_image_registry /
  master_audio_registry に存在しなければ pending entry を提案。
  vocab / example / materialNeeds.keywords (括弧説明は strip) の 3 経路を走査。
  既存 entry には usedInLessons に NN を追加提案。dry-run 既定

**X-a-3 で見つかった既存課題** (本セッションでは未修正・user 判断待ち):
