...
NATIONALITY_NOUN_POLICY = {
  ...
  "flag_pin_rule":
    ...
    # v3.12 (PART 1.7 FLAG_PLACEMENT_RULE): 位置は universal options から word
    # hash で deterministic 選択するため、本記述からは位置を削除。size 規律のみ残す。
    # placement は subject_block_pattern の {FLAG_PLACEMENT} placeholder に注入される。
    "Render the flag as a SMALL circular pin or small rectangular cloth "
    "patch. The flag must occupy about 5-6% of the image area (slightly "
    "larger than v3.6 to improve readability without dominating the frame). "
    "It is a subtle secondary identifier, NOT the primary subject. "
    "Placement on the garment is selected per FLAG_PLACEMENT_RULE "
    "(PART 1.7) and injected via the {FLAG_PLACEMENT} placeholder in "
  ...
  "hard_constraints": [
    ...
    # v3.9: NATIONAL_SYMBOL_ISOLATION_RULE（普遍ルール）。
    #   背景：v3.8 実機検証で米 (graphic t-shirt → 国旗プリント) と
    #   ブラジル (green/yellow football palette → 旗類似/CBF クレスト) で
    #   「衣服自体が flag に侵食される」事象が確認された。flag pin/patch を
    #   唯一の national-symbol として孤立させ、衣服・バッグ・アクセサリ・
    #   garment 色配列・unspecified graphic への侵食を全方面で禁止する。
    #   将来国（タイ・印・仏・独 等）の追加でも同種事故を再発させないため、
    #   per-country patch ではなく普遍 hard_constraint として明文化する。
    "NATIONAL_SYMBOL_ISOLATION_RULE (v3.9 新規・普遍ルール): "
    "The single flag pin/patch (5-6% of frame, on chest or bag strap) is "
    "the ONLY permitted national-symbol occurrence in the image. The "
    "following are universally PROHIBITED across all current and future "
    "countries: "
    ...
    "yellow accessory for a green-yellow flag country — the flag pin "
    "already carries this signal and duplication produces flag-like "
    "imagery on the body). "
    ...
    "(e.g., subtle stripes, dots, abstract shapes, botanical print, or "
    "a well-defined traditional pattern such as wagara whose semantic "
    "content is explicitly NOT the national flag).",
  ],
  ...
  "subject_block_pattern":
    ...
    # v3.12: {APPARENT_FEATURES_HINT} は PHENOTYPE_SPECIFICATION_RULE (PART 1.5)
    #        準拠で phenotype_for(word) が単一 concrete 記述を供給。
    #        {CULTURAL_STYLING_HINT} は TRADITIONAL_DRESS_PATTERN_RULE (PART 1.6)
    #        準拠で pattern_for(word) が pattern element を append する場合あり。
    #        {FLAG_PLACEMENT} は FLAG_PLACEMENT_RULE (PART 1.7) 準拠で
    #        flag_placement_for(word) が deterministic に位置を供給。
    ...
    "of the frame) is positioned {FLAG_PLACEMENT} as a subtle nationality cue. "
  ...
  "apparent_features_hint_definition":
    # v3.12: enumerate アプローチが Imagen / nanobanana で中央値収束を起こす
    # ことが実機検証で確定したため、PHENOTYPE_SPECIFICATION_RULE (PART 1.5)
    # 準拠の単一 concrete 記述に切替。
  ...
  # v3.11 新規: TWO-COLOR RULE の asymmetric 適用に関する rationale
  # （build_prompts.py PERSON_NATIONALITY_HINTS でアジア 4 か国のみ
  # TWO-COLOR RULE を持っている理由を明文化）。
  "two_color_rule_asymmetric_rationale":
    "v3.10 で PERSON_NATIONALITY_HINTS のアジア 4 か国（日本人 / 中国人 / "
    "韓国人 / ベトナム人）の cultural_styling_hint にだけ TWO-COLOR RULE を "
    "追加した。これは「漏れ」ではなく「構造的に意図された asymmetric」である。 "
    "理由：アジア 4 か国の cultural element は単一文化 garment（qipao top, "
    "hanbok-inspired top, áo dài, noragi 等）が全身を占める設計のため、明示的 "
    "rule 無しでは Imagen が「安全側の muted gray-blue 単色」に収束する "
    "（v3.9 実機検証で確認）。一方、欧米南米 3 か国（アメリカ / ブラジル / "
    "スペイン）は複数 garment 構成（chore coat + t-shirt + jeans, "
    "linen shirt + tee + chinos + espadrille, botanical print top + trousers "
    "等）で自然に多色化するため、明示的 TWO-COLOR RULE 無しでも視覚多様性が "
    "確保される。両者に普遍的に同じ rule を課すと、欧米南米には冗長な縛りに "
    "なり cultural_styling_hint の記述コストが上がる。よって本 asymmetric は "
    "保持する。 "
    "将来 audit 時の判断指針：新規 nationality を追加する際、その国の "
    "cultural styling が「単一文化 garment が全身を占める設計」であれば "
    "TWO-COLOR RULE を per-country で追加し、「複数 garment 構成」であれば "
    "TWO-COLOR RULE は不要（garment composition が代替する）。 "
    "VISUAL_CONTRAST_PRINCIPLE.sub_rules_by_situation との関係：本 asymmetric "
    "は universal な visual_contrast_principle の派生適用ではなく、cultural "
    "styling の構造特性に応じた per-domain 判断。両者は直交。",
}
...
## ============================================================
## PART 1.6: TRADITIONAL_DRESS_PATTERN_RULE（伝統衣装パターン必須化規律）
## ============================================================
## v3.12 新規。v3.10 で アジア 4 か国の cultural_styling_hint に TWO-COLOR
## RULE を追加し top と trousers の二色化を強制したが、textile pattern /
## embroidery の必須化は未対応だった。結果として実機検証で「modernized
## qipao-inspired top in soft jade」のような silhouette 記述があっても、
## Imagen / nanobanana が「ただ無地のスタンドカラーブラウス」として描画し、
## 文化的弁別性が失われるケースが残った。
##
## 構造的解決：cultural traditional silhouette を指定する際は、その silhouette
## に固有の visible textile pattern / embroidery / accent を MUST として 1 つ
## 含める。pattern は country ごとに 1 つだけ（複数 enumerate しない =
## PART 1.5 rule_a 整合）、scripts/build_prompts.py の
## TRADITIONAL_DRESS_PATTERN_LOOKUP に格納。
##
## NATIONAL_SYMBOL_ISOLATION_RULE との関係：
##   national-flag motif / 国家エンブレム / チームクレストは禁止のまま。本
##   ルールが許可する pattern は「文化的織物パターン / 刺繍 / アクセント」で、
##   national-symbol とは別カテゴリ。両者は直交。
##
## 非ラテン文字の禁止：
##   プロンプト本文に kanji / hangul / 非ラテン文字を含めない（画像生成器が
##   描画できず文字化け、または「textile に文字が描かれる」事故の原因）。
##   pattern 名は英語の motif 名称（asanoha / seigaiha / wagara / peony /
##   plum-blossom / crane / pine-branch 等）でのみ記述する。
##
TRADITIONAL_DRESS_PATTERN_RULE = {
  "principle":
    "When a cultural traditional silhouette is specified in a nationality-"
    "noun person card (e.g., modernized qipao-inspired top, hanbok-inspired "
    "modern top, áo dài, noragi-inspired jacket, modernized cheongsam), the "
    "prompt MUST include exactly ONE visible textile pattern, embroidery, "
    "or accent element drawn from the country's traditional design "
    "vocabulary. Without this element, generators render a plain solid-color "
    "modern garment that loses cultural identifiability.",

  "rule_a_single_concrete_element":
    "Specify exactly ONE visible textile element per card (not a list of "
    "options) — consistent with PART 1.5 rule_a's no-enumerate principle. "
    "The element is supplied per country by TRADITIONAL_DRESS_PATTERN_LOOKUP "
    "(scripts/build_prompts.py). Future-version expansion can rotate the "
    "element per word via the same deterministic-hash mechanism, but v3.12 "
    "uses a single fixed element per country.",

  "rule_b_visible_placement":
    "The pattern must be VISIBLE in the rendered image — i.e., placed on a "
    "garment area that is in-frame and not occluded by another element. "
    "Acceptable placements: front body, chest, collar, cuff, hem, side-slit "
    "border, shoulder, sleeve. Avoid back-only or tag-only placements that "
    "would not show in a 3/4 view.",

  "rule_c_english_motif_names_only":
    "Pattern names in prompts must use English motif vocabulary ONLY — no "
    "kanji, no hangul, no non-Latin script. Imagen / nanobanana cannot "
    "render non-Latin characters and tend to either produce mojibake on the "
    "textile or skip the element entirely. "
    "GOOD: 'subtle wagara textile pattern (asanoha hemp-leaf or seigaiha "
    "wave)' — English motif names. "
    "BAD: '伝統的な和柄' — Japanese in prompt; generator may write 和柄 "
    "as literal characters onto the cloth.",

  "rule_d_no_flag_conflict":
    "The pattern must NEVER incorporate the country's national-flag motif "
    "or color sequence in flag-like proportions (per "
    "NATIONAL_SYMBOL_ISOLATION_RULE). Traditional cultural patterns (asanoha, "
    "seigaiha, peony, plum-blossom, crane, pine-branch, hibiscus, embroidered "
    "borders, etc.) are explicitly different from national-flag symbols and "
    "are permitted.",

  "rule_e_modern_styles_exempt":
    "Modern (non-traditional) outfit choices are EXEMPT from this rule — "
    "if the cultural_styling_hint picks a contemporary fashion pattern "
    "(e.g., American chore coat + jeans, Brazilian botanical-print linen, "
    "Spanish espadrille casual), no traditional textile element is required. "
    "The lookup TRADITIONAL_DRESS_PATTERN_LOOKUP may be sparse — countries "
    "whose cultural_styling_hint emphasizes modern silhouettes need not "
    "have an entry. v3.12 populates the lookup primarily for the 4 Asian "
    "countries (日本人 / 中国人 / 韓国人 / ベトナム人) where the cultural "
    "silhouette is most likely to default to plain solid color.",

  "data_location_note":
    "TRADITIONAL_DRESS_PATTERN_LOOKUP (country word → 1 concrete English "
    "motif description) lives in scripts/build_prompts.py, NOT in this "
    "guide. The guide states the universal RULE; the lookup is a small "
    "data table that future lesson designers extend as new countries are "
    "added.",

  "enforcement":
    "build_prompts.py の pattern_for(word) ヘルパーが本ルールを適用し、 "
    "cultural_styling_hint の末尾に 'TRADITIONAL DRESS PATTERN (v3.12 "
    "PART 1.6): the garment MUST visibly include {pattern}.' を append する。"
    "lookup に entry が無い country では何も append しない（rule_e）。"
    "pre-flight invariant としての mechanical check は v3.12 段階では実装しない。",
}


## ============================================================
## PART 1.7: FLAG_PLACEMENT_RULE（国旗ピン位置の deterministic 選択規律）
## ============================================================
## v3.12 新規。v3.11.1 までは NATIONALITY_NOUN_POLICY.subject_block_pattern に
## "on the chest or bag strap" と曖昧に書いていたため、7 か国カードを並べる
## と Imagen / nanobanana が全員「左胸の小ピン」に画一化し、視覚的単調に
## なる事象が確認された。pedagogical な弁別性（phenotype + cultural styling）
## とは独立だが、教材品質としての視覚多様性が落ちる。
##
## 構造的解決：flag pin/patch の位置は universal な 4 option（左胸ピン /
## 右胸ピン / 右袖パッチ / 左袖パッチ）から、word hash で deterministic に
## 選択する。garment 上の選択肢のみに限定（hat / bag / book / shoe 等を
## 新たに生やさない = 既存のコンポジションを壊さない）。
##
## NATIONAL_SYMBOL_ISOLATION_RULE との関係：
##   flag pin/patch は引き続き「the ONLY permitted national-symbol」。本
##   ルールはその「唯一の flag occurrence」の位置を決めるだけで、追加の
##   flag motif を許可するものではない。両者は補完的。
##
## flag_pin_rule（PART 1.1）との関係：
##   サイズ規律（5-6% of frame）は変更なし。本ルールは位置のみを決める。
##   flag_pin_rule の "on the chest or on a bag strap" 表現は v3.12 で
##   削除し、本ルールが供給する {FLAG_PLACEMENT} placeholder に置き換える。
##
FLAG_PLACEMENT_RULE = {
  "principle":
    "The single national-flag pin/patch's placement on the garment is "
    "selected from a fixed universal set of 4 placements via deterministic "
    "word hash, so that 7 nationality cards within a single lesson "
    "visibly differ in flag location, while the same word always yields "
    "the same placement across regenerations (reproducibility).",

  "rule_a_garment_only":
    "Placements are restricted to positions on the outer garment (chest, "
    "sleeve, shoulder). The rule MUST NOT introduce new accessory surfaces "
    "(hat, bag, book cover, shoes, glasses) that the base composition does "
    "not already include — adding such surfaces grows the scene "
    "unpredictably and violates the SCENE 'no other objects' constraint "
    "of vocabulary_person template.",

  "rule_b_deterministic_selection":
    "Placement = FLAG_PLACEMENT_OPTIONS[sha256(word + 'flag-placement') "
    "mod 4]. The salt 'flag-placement' is chosen distinctly from "
    "PART 1.5 rule_d's 'phenotype' salt so that flag placement and "
    "phenotype selection are statistically independent (uncorrelated "
    "buckets across a lesson's 7 nationality words).",

  "rule_c_single_occurrence":
    "Exactly ONE flag occurrence per card, at exactly ONE placement (per "
    "NATIONAL_SYMBOL_ISOLATION_RULE). The rule does NOT add a second "
    "flag, even on a different garment area.",

  "rule_d_size_unchanged":
    "Size regulation (5-6% of frame, per PART 1.1 flag_pin_rule) is "
    "unchanged. This rule governs placement only.",

  "data_location_note":
    "FLAG_PLACEMENT_OPTIONS (4 string entries, each a complete placement "
    "phrase that drops into the subject_block_pattern in place of the "
    "former 'on the chest or bag strap' text) lives in scripts/"
    "build_prompts.py, NOT in this guide. The 4-option set is frozen at "
    "v3.12; adding a 5th option would re-hash all existing words and "
    "break reproducibility (treat as v4.0+ migration, per PART 1.5 "
    "rule_e palette-freeze principle).",

  "hash_distribution_check":
    "After implementation, verify that the 7 nationality words in "
    "lesson_01 (日本人 / 中国人 / アメリカ人 / 韓国人 / ブラジル人 / "
    "ベトナム人 / スペイン人) distribute across the 4 buckets without "
    "extreme skew (e.g., 6 of 7 in one bucket). Adjust salt string if "
    "needed before v3.12 freeze.",

  "enforcement":
    "build_prompts.py の flag_placement_for(word) ヘルパーが本ルールを実装し、 "
    "NATIONALITY_NOUN_POLICY.subject_block_pattern の {FLAG_PLACEMENT} "
    "placeholder に注入する。pre-flight invariant としての mechanical check は "
    "v3.12 段階では実装しない。",
}
