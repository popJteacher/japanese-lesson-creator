...
# Version 4.0 | 2026-05-22 (major-version pivot: 全国 modern daily casual wear + 国旗両手持ち)
#   v3.12 まで「アジア 4 か国（日中韓越）は伝統 silhouette + textile pattern」「西洋 3 か国
#   （米伯西）は modern garment + regional craft accent」という構造的アシンメトリで設計されていた。
#   これは以下の問題を抱えていた：
#     (P1) 暗黙の二分（アジア=伝統服 / その他=現代服）が exoticization リスクを孕む
#     (P2) modern reality（実際の現地人は日常的に伝統服を着ない）と乖離
#     (P3) 教科書 nationality 挿絵の慣用（『みんなの日本語』『げんき』等は全員 modern wear
#          + 国旗 / 名札 で nationality を伝える）と異なる
#   worktree image-prompt-plan セッション（2026-05-22）で v3.13-#1 GARMENT_REGISTER_CONSISTENCY_RULE
#   着手前にこの asymmetry が論点化。逆方向の simplification = 全国共通 modern wear + 国旗視認性
#   強化への pivot を確定。
#
#   v4.0 設計核心:
#     (1) 全国共通 "modern daily casual wear" に統一。silhouette / fit / accessory tendency で
#         国別 subtle 弁別、palette は country-today の modern fashion tendency に基づく
#         （exoticization 回避・伝統服 cliché 回避）。
#     (2) 伝統 silhouette は universal NO_TRADITIONAL_SILHOUETTE_RULE で明示禁止（PART 1.1
#         hard_constraints）。hanbok / qipao / áo dài / yukata / wagara-cut kimono / cheongsam /
#         jeogori / noragi / 祭礼衣装 すべて禁止。modernized form も禁止。
#     (3) 国旗は subject が両手で胸前で持つ hand-held flag pose（~12-15% of frame, flag face
#         squarely toward viewer）に切替。v3.12 の「衣服上の小さな pin/patch 5-6%」は退役。
#         east_asian phenotype を共有する日中韓 3 か国の弁別が、衣服 silhouette + flag pose の
#         組合せで明確になる設計。
#     (4) PART 1.6 TRADITIONAL_DRESS_PATTERN_RULE は退役（伝統 silhouette が消えたため原理的
#         に不要）。ファイル末尾に退役マーカー 1 段落のみ残す。
#     (5) PART 1.7 FLAG_PLACEMENT_RULE は簡素化（全員固定 pose・deterministic-pick 不要）。
#     (6) PART 1.5 PHENOTYPE_SPECIFICATION_RULE は v4.0 でも維持。palette_freeze は継続
#         （PHENOTYPE_PROFILES / ROLE_PHENOTYPE_PALETTE / PHENOTYPE_SALT を変更しない）。
#         major-version migration を機会としつつ phenotype は変える必然性が無いため温存。
#     (7) NATIONAL_SYMBOL_ISOLATION_RULE は v4.0 でも core 規律として維持。flag 記述のみ
#         「pin/patch 5-6%, on chest or bag strap」→「hand-held flag 12-15%, held in both
#         hands in front of the chest」に書き換え。衣服へのフラッグプリント禁止 (a)/(c)/(d) は
#         むしろ v4.0 で強化される（hand-held flag があるからこそ衣服は plain solid であるべき）。
#     (8) PERSON_NATIONALITY_HINTS は scripts/build_prompts.py 内で全 7 国を B-rich 形式に
#         書き直し（silhouette/fit/accessory tendency + 国別 muted palette tendency + 「伝統
#         silhouette NEVER」は universal rule 経由）。
#     (9) compose_nationality_pose は「arms naturally at the sides」→「arms bent at the elbows
#         holding the hand-held flag in front of the chest」に書き換え。
#
#   PHASE_BACKLOG v3.13 候補の扱い（2026-05-22 pivot 反映）：
#     - #1 GARMENT_REGISTER_CONSISTENCY_RULE: v4.0 で原理的に不要（伝統 register が消えたため）→ retire
#     - #2 role cards plain-solid 明文化: v4.0 で副次的に解消見込み（modern wear 統一で role plain-solid
#          が自然に成立しやすい）→ v4.0 後に実害観測されたら再評価
#     - #3 伝統服が薄い／無い国の signature 多軸化: v4.0 が逆方向で解消（multi-axis lookup → 単一
#          modern wear + 国旗強化）→ retire
#     - #4 vocab_subtype: v4.0 とは独立 → backlog として保持
#
#   v4.0 で退役・変更されたが情報価値があるもの:
#     - PART 1.6 TRADITIONAL_DRESS_PATTERN_RULE (退役。本ファイル末尾セクション参照)
#     - PART 1.7 FLAG_PLACEMENT_RULE (4 option deterministic-pick → 全員固定 pose に簡素化)
#     - TWO_COLOR_RULE asymmetric rationale (v3.10 で導入したアジア限定 TWO-COLOR rule の理由づけ・削除)
#     - 各国 cultural_styling_hint 内の伝統 silhouette option + textile pattern 記述
#       (archive/prompts/master_prompt_design_guide_v3_12.py 参照)
#
#   関連:
#     - docs/MIGRATION_PLAN.md § v4.0 マスタープロンプトガイド major-version 改修
#     - docs/PHASE_BACKLOG.md § v3.13 マスタープロンプトガイド修正候補 (v4.0 pivot 反映済)
#     - handoff_archive.md § 方針転換ログ 2026-05-22
#     - memory: project_v4_0_pivot.md (worktree 側 / main 側 同名)
#
#   Rollback 手順（v4.0 で重大問題発生時）：
#     scripts/build_prompts.py の GUIDE_PATH を
#       'prompts/master_prompt_design_guide_v3_12.py' に戻す
#     scripts/invariants.mjs の promptGuide / promptGuideExpectedHashPrefix を
#       v3.12 の値 (2137a8e885ae) に戻す
#     scripts/invariants.mjs の sColumnPattern を `/^image_prompts_lesson\d{2}_v3_\d+(_\d+)?\.json$/` に戻す
#     再生成・validate → v3.12 完全復帰。本 v4.0 ファイルは保持
#     （将来 v4.0 設計の参照 / 比較が必要な場合に再利用）。
#
# Version 3.12 | 2026-05-21 (= v3.11.1 + 3 universal rules: PHENOTYPE_SPECIFICATION_RULE / TRADITIONAL_DRESS_PATTERN_RULE / FLAG_PLACEMENT_RULE)
#   v3.11.1 実機検証（vocab_person 5 件 smoke + 過去 lesson_01 12 件分の知見）と
#   nanobanana 固定運用への切替に伴い、以下 3 問題が普遍的構造として確定：
#     (Z1) {APPARENT_FEATURES_HINT} の "Pick ONE specific set of features from:
#          (skin tone: A OR B OR C OR D) + (hair: X OR Y OR Z) + (texture: ...)"
#          enumerate 記述は、単一画像生成では Imagen / nanobanana が選択肢の
#          中央値（"ethnically ambiguous medium tone"）に倒れ、結果として
#          複数人を生成しても phenotype が画一化する。これは v3.8 以降一貫して
#          露呈してきた構造欠陥で、v3.11.1 でも未解決。
#     (Z2) アジア 4 か国（日中韓越）の cultural traditional silhouette を
#          指定しても、textile pattern / embroidery が無いと「ただ無地の
#          modern top」として描画され、文化的弁別性が失われる。v3.10 で
#          TWO-COLOR RULE は追加したが、pattern element の必須化は未対応。
#     (Z3) 国旗ピンの位置を "on the chest or bag strap" と曖昧に書いていたため、
#          7 か国カードを並べると flag position も全員「左胸ピン」に画一化し、
#          視覚的単調になる（pedagogical な弁別性とは独立だが、教材品質として
#          単調）。
#
#   v3.12 修正（drop 候補との対比は docs/PHASE_BACKLOG.md § v3.12 修正候補参照）：
#     (1) PART 1.5 PHENOTYPE_SPECIFICATION_RULE（新規普遍ルール）を追加。
#         (a) enumerate ("Pick ONE from: A / B / C") は全面禁止（rule_a）、
#         (b) 隣接 2 段階までの range は許容（rule_b, "fair to light-medium"）、
#         (c) nationality nouns は COUNTRY_TO_PROFILE → PHENOTYPE_PROFILES で
#             1 つの concrete 記述（rule_c）、
#         (d) role nouns は ROLE_PHENOTYPE_PALETTE から sha256(word+'phenotype')
#             mod N で deterministic 選択（rule_d）、
#         (e) パレットは v3.12 で fix。変更は major-version migration（rule_e）。
#         データ：PHENOTYPE_PROFILES (7) / COUNTRY_TO_PROFILE / ROLE_PHENOTYPE_PALETTE (6)
#         は scripts/build_prompts.py に置く。americas_diverse profile は
#         多民族国家の扱いとして ROLE_PHENOTYPE_PALETTE にフォールバック。
#     (2) PART 1.6 TRADITIONAL_DRESS_PATTERN_RULE（新規普遍ルール）を追加。
#         Cultural traditional silhouette を指定する際は MUST 1 つの visible
#         textile pattern / embroidery / accent を含む。
#         データ：TRADITIONAL_DRESS_PATTERN_LOOKUP（country → 1 concrete pattern）
#         は scripts/build_prompts.py に置く。プロンプト本文に kanji / hangul /
#         非ラテン文字を含めない（英語のみで motif 名称）。
#     (3) PART 1.7 FLAG_PLACEMENT_RULE（新規普遍ルール）を追加。
#         flag pin/patch の位置は universal options から word hash で
#         deterministic 選択。garment 上のみに限定（hat/bag/book を新たに
#         生やさない）。
#         データ：FLAG_PLACEMENT_OPTIONS (4 entries) は scripts/build_prompts.py
#         に置く。選択は sha256(word+'flag-placement') mod 4。
#     (4) PERSON_NATIONALITY_HINTS の apparent_features_hint 静的データは
#         scripts/build_prompts.py 側から削除し、phenotype_for(word) 関数で
#         動的計算する。
#     (5) v3.11.1 の ASPECT RATIO 1:1 directive は両モデル安全のため保持。
#         AI Studio / Gemini / nanobanana いずれでも問題ない。
#
#   drop 候補（v3.12 では対応しない）:
#     #1 N=1 確率揺れ（再生成で解消）/ #4 実害小 / #6 教育的に不要 /
#     #7 #8 #9 Imagen 4 固有 → nanobanana 固定運用に切替したため不要
#
# Version 3.11.1 | 2026-05-21 (TEMPORARY: Gemini fallback 専用 / v3.11 + inline ASPECT RATIO 1:1 directive)
#   背景：AI Studio が一時的に使用不可となり、画像生成を Gemini (consumer chat) に
#         切り替えざるを得ない状況。Gemini は API パラメータでアスペクト比を指定
#         できず、プロンプト本文中に明示する必要がある。AI Studio (`--aspect 1:1`)
#         復活までの暫定 patch として、vocabulary_person template の SCENE 句
#         冒頭に「ASPECT RATIO: 1:1 SQUARE」directive を inline する。
#
#   v3.11.1 修正：
#     (1) PROMPT_TEMPLATES["vocabulary_person"] の [SCENE & ACTION] 冒頭に
#         「ASPECT RATIO (v3.11.1 TEMPORARY for Gemini fallback): The output
#         image MUST be 1:1 SQUARE — equal width and height. DO NOT output
#         16:9 widescreen, 4:3 landscape, 3:4 portrait, or any other ratio.
#         Square 1:1 only.」を追加。
#     (2) 他テンプレート（building 等）は本 patch スコープ外。必要なら同じ
#         pattern で個別追加。
#
#   Rollback 手順（AI Studio 復活後）：
#     scripts/build_prompts.py の GUIDE_PATH を
#       'prompts/master_prompt_design_guide_v3_11.py' に戻す
#     scripts/invariants.mjs の promptGuide / promptGuideExpectedHashPrefix を
#       v3.11 の値 (29407f70fc19) に戻す
#     再生成・validate → v3.11 完全復帰。本 v3.11.1 ファイルは保持
#     （将来 Gemini fallback が再発したときに再利用）。
#
# Version 3.11 | 2026-05-21 (= v3.10 + PROMPT_LITERALIZATION_AVOIDANCE_RULE 普遍化 + footwear-mandatory rule + teacher lanyard 修正 + 日本人 yukata at-home 削除 + TWO-COLOR asymmetric rationale)
#   v3.10 実機検証で 4 問題が露呈：
#     (V) teacher lanyard に「rectangular plastic ID badge」を意図して
#         "a small generic icon such as a book or pencil silhouette" と
#         書いたところ、Imagen が "such as X or Y" を「draw X or Y」と
#         literalize し、首から鉛筆マーク付きペンダントが下がる絵に。
#         単一 role 固有ではなく、ガイド全体に潜在する汎用パターン
#         （Imagen が example/hint を draw instruction として literalize する）の
#         一つの顕在化。
#     (W) 日本人画像が裸足で出力。cultural_styling_hint option (c)
#         "an everyday yukata in a summer-at-home context" の "at-home"
#         が semantic leak し、選択された option (b) noragi-inspired
#         outfit にも「家にいる→靴を履かない」を partial application。
#         他アジア 3 か国（中韓越）には yukata 相当の "at-home" option が
#         無いため全員 footwear 着用 → 日本のみ。
#     (X) アメリカ人画像のみ 0 度正面 view（他 9 件は 3/4 view）。
#         vocabulary_card_focal_length ルール自体は機能している（9/10 PASS）
#         ため、1 件の確率揺れと判断し v3.11 では構造修正しない。
#         再現性確認後に判断。
#     (Y) 外国人 (foreigner) signature は v3.10 で phrasebook + crossbody
#         bag に強化したが、検証バッチに含まれず未確認。v3.11 では未対応、
#         検証後に判断。
#
#   v3.11 修正：
#     (1) PART 1.4 PROMPT_LITERALIZATION_AVOIDANCE_RULE（新規普遍ルール）を
#         追加。「Imagen は prompt 内の example / hint / placeholder を
#         そのまま literalize する」前提でプロンプトを書く。
#         (a) "such as / e.g. / for example" は「Imagen が example をそのまま
#             描く」前提でのみ使う。blank/placeholder 意図のときは絶対に
#             example を書かない。
#         (b) "may / might / optional / possibly" は省略リスク高。
#             MUST / NEVER に統一（v3.10 ROLE_VISUAL_IDENTITY_RULE.
#             must_carry_language_rule を role 限定から普遍化）。
#         (c) Vague descriptor（"a small icon" 等）は具体化（"a blank
#             rectangle" / "a plain badge with no marking" 等）。
#     (2) teacher.outfit_hints の lanyard 記述を本ルール準拠で書き直し：
#         「a rectangular plastic name-tag-style ID badge clipped to a
#         thin strap or cord at chest level — the badge is a plain blank
#         rectangle with NO text, NO icon, NO graphic. The rectangular
#         shape alone identifies it as a generic ID badge.」
#     (3) PERSON_NATIONALITY_HINTS["日本人"] yukata option から "at-home"
#         を削除し「an everyday yukata in a daily summer setting」に変更。
#         room context を消すことで semantic leak を防止。
#     (4) PROMPT_TEMPLATES["vocabulary_person"] の SCENE 句に普遍 footwear-
#         mandatory rule を追加：「Both feet MUST visibly wear footwear
#         (shoes, sandals, espadrilles, ballet flats, work boots, sneakers,
#         or similar). Barefoot is NOT permitted in vocabulary cards.」
#     (5) NATIONALITY_NOUN_POLICY に TWO-COLOR asymmetric rationale note
#         を追加。なぜアジア 4 か国だけに TWO-COLOR RULE があるのか
#         （アジア = 単一文化 garment が全身を占める → 単色化リスク高、
#         西洋 = 複数 garment 構成で自動的に多色 → 明示不要）を明文化。
#         将来の audit/修正者が「漏れ」と誤認しないため。
#     (6) アメリカ正面・外国人 signature は v3.11 では未対応。NEXT_ACTIONS 参照。
#
# Version 3.10 | 2026-05-21 (= v3.9 + ROLE_VISUAL_IDENTITY_RULE 普遍化 + VISUAL_CONTRAST_PRINCIPLE 普遍化 + アジア 4 か国 二色化必須化)
#   v3.9 実機検証（lesson_01 person 全 12 件）で 3 問題が露呈：
#     (S) 役割系の visual identity 不均衡。doctor (白衣)・company_employee (suit)・
#         student (backpack) は garment / prop signature が強くロール即読み取り可
#         だが、teacher (generic smart-casual + "may carry" textbook) と
#         foreigner (同) は signature が弱くロール判別不能。原因は v3.7 で
#         ISOLATION_SAFE_PROPS_RULE により pointer (teacher の最強 signature) を
#         削除した時点で teacher の visual ID が落ちていたこと + outfit_hints の
#         "may carry" 表現が Imagen に prop 省略を許していたこと。teacher 固有
#         ではなく将来新 role 追加（警察官・店員・運転手 等）でも再発する
#         構造的問題。
#     (T) アジア 4 か国（日中韓越）が「全身 muted gray-blue 単色」に収束。
#         ブラジル・スペイン・アメリカは複数色（print / chinos / espadrille
#         / chore coat + 黄色 t-shirt 等）で出るのに、アジア圏は v3.9 で
#         "muted color palette" "simple solid muted color" "in muted modern
#         colors" を繰り返し強調しすぎて、Imagen が「安全側の単色」に収束。
#         結果アジア 4 か国の弁別性が落ち、ブラジル・スペインだけが色彩的に
#         浮く不均衡。
#     (U) v3.9 で導入した prop contrast rule が compose_role_subject の中だけに
#         閉じ込められ、他 vocab_type（concrete_object / adjective PAIR_CONTRAST
#         / abstract_concept / spatial_relation / building）から見えず、STYLE_BIBLE
#         にも対比規律が一切明文化されていない。将来 vocab_type 実装で同種
#         「色被り埋没」事象が再発する構造的欠落。
#
#   v3.10 修正：
#     (1) PART 1.3 ROLE_VISUAL_IDENTITY_RULE（新規普遍ルール）を追加。各 role
#         は「単独識別可能な visual signature」を必須とする：(a) garment-level
#         signature（白衣・スーツ・制服 等）OR (b) mandatory prop（isolation-safe
#         で必須着用/必須携帯）OR (c) 両者の組み合わせ。"may carry" 表現を
#         universally 禁止し "MUST visibly carry/wear" に統一。ROLE 間 signature
#         重複を避ける。将来新 role 追加でも本ルール準拠で書くため再発しない。
#     (2) ROLE_BASED_GENERIC_PROFILES audit：
#         - teacher: outfit_hints に「MUST wear a lanyard name badge (no text,
#           small icon only) at chest level」「MUST visibly hold a thick visible
#           textbook with visible spine in one hand or under one arm」を追加。
#           cardigan/jacket は維持。これで garment+prop で唯一識別可能に。
#         - foreigner: 「MUST hold a Japanese language phrasebook or textbook
#           with the cover visibly facing the viewer (no text, just generic
#           book design)」「a small day bag or shoulder strap visible」を追加。
#         - student / doctor / company_employee: "may carry" を "MUST visibly
#           carry/wear" に書き換え（既に signature は強いが言語統一）。
#     (3) STYLE_BIBLE.visual_contrast_principle（新規普遍ルール）を STYLE_BIBLE
#         直下に追加。「画像内の主要素同士は STYLE_BIBLE.color_palette 内の
#         異なる色で対比し、視認性を確保する」。具体規律：
#         (a) 人物 prop は服装支配色と異なる hue family AND/OR value、
#         (b) PAIR_CONTRAST 2 物体は palette 内の異なる main/accent/sub で
#             視覚分離、
#         (c) 物体カード単体は背景クリームに対して main_color/accent/sub_color
#             のいずれかで対比、
#         (d) symbolic 色（symbol_red 等）は意味で選び、対比は視認性で選ぶ
#             ため両者直交。
#         compose_role_subject の prop contrast rule を本普遍ルールから
#         derive する形にリファクタ（記述を「rule derived from
#         STYLE_BIBLE.visual_contrast_principle」と参照表現に変更）。
#     (4) PERSON_NATIONALITY_HINTS のアジア 4 か国（日中韓越）cultural_styling_hint
#         に「top + trousers の二色化 OR top に明示的なアクセント色（襟・帯・
#         otgoreum・縁取り）」を必須化。旗色 overlap 回避を per-country で明示：
#         - 日本: top が wagara なら trousers は muted indigo / charcoal / sage。
#           white top + red trousers / red top + white trousers は flag-like
#           で禁止。
#         - 中国: top と trousers は別色（red+yellow combo は既禁則）。
#         - 韓国: hanbok-inspired top に異色 otgoreum ribbon、trousers は
#           cream / charcoal / sage 等。
#         - ベトナム: áo dài の body color と trousers color が異なる
#           （伝統的にもそう）。red body + 中央 yellow ornament は既禁則。
#     (5) 会社員背景純白問題は v3.9 で 1 件のみの確率揺れの可能性が高く、
#         構造修正は保留。人間再生成で確認後に判断（NEXT_ACTIONS 参照）。
#
# Version 3.9 | 2026-05-21 (= v3.8 + NATIONAL_SYMBOL_ISOLATION_RULE 普遍化 + cultural_styling_hint 必須要素方式 + role prop palette-aware contrast rule)
#   v3.8 実機検証（lesson_01 person 全 12 件）で 3 問題が露呈：
#     (P) 国籍系 cultural_styling_hint の option (a)「modern X casual」が
#         全 7 か国で常に Imagen に選ばれ、結果 5/7 か国（米・韓・中・スペイン・
#         一部ブラジル）が「t-shirt + casual pants + 旗ピン」の同型に収束。
#         option (b)/(c) が無い／弱い国だけ文化要素が出た（日: wagara 羽織風、
#         ベトナム: áo dài）。3 option 並列のうち最 safe な選択肢に常時収束する
#         構造そのものが原因。
#     (Q) アメリカ人の cultural_styling_hint(a) に "graphic t-shirt" と書いた
#         ところ、Imagen が「graphic = American graphic」と解釈し国旗を Tシャツ
#         前面にプリント。結果「旗ピン (5-6%) + Tシャツ前面の旗プリント」の
#         二重旗。hard_constraints に「衣服自体に旗モチーフを描画してはならない」
#         という明示禁則が無かったための漏れ。
#     (R) 役割系 (teacher / student 等) で「同色相 prop と outfit」現象。
#         例: light-blue cardigan + light-blue marker pen。compose_role_subject
#         も outfit_hints も「色対比」を要求していないため、Imagen はパレット
#         範囲内で最も馴染む色を選び、結果 prop が outfit に埋没する。
#
#   v3.9 修正：
#     (1) PART 1.1 NATIONALITY_NOUN_POLICY に NATIONAL_SYMBOL_ISOLATION_RULE
#         （新規普遍ルール）を追加。「flag pin/patch is the ONLY permitted
#         national-symbol」「衣服・バッグへの旗モチーフ／プリント／チーム
#         クレスト禁止」「flag-like 色配列 (red+yellow が国旗の国で red top +
#         yellow trousers 等) 禁止」「unspecified-content の graphic/printed
#         garment 禁止」の 4 項を明文化。将来国（タイ・印・仏・独 等）追加
#         でも同種事故を再発させない普遍ルール。
#     (2) PERSON_NATIONALITY_HINTS（build_prompts.py 側）全 7 か国の
#         cultural_styling_hint を「3 option pick ONE」から「必須要素方式」に
#         再構成：modern X casual を全削除し、cultural element を必須として
#         2-3 個の concrete 選択肢を提示。Imagen は文化要素を必ず含む 1 つを
#         discrete に選ぶ。
#     (3) アメリカ人: "graphic t-shirt" → "plain solid-color t-shirt OR
#         non-flag/non-team patterned top (abstract shapes, subtle stripes,
#         botanical print)"。
#     (4) ブラジル人: 全削除し再構成。緑黄 football-supporter pallete option
#         (旗色 + sports adjacency = 旗類似/CBF クレスト混入の高リスク) を撤去。
#         代替: 「Brazilian botanical/leaf print top in earthy natural tones」
#         「Brazilian linen short-sleeve over tank top」「modern casual with
#         a small woven-cord/bead bracelet (cultural craft accent)」。
#     (5) 中国人 / スペイン人: 「subtle warm red or yellow accent」(旗ピン
#         red+yellow と合わさり旗類似化するリスク) を撤去し、「avoid
#         red+yellow combination; muted earth-tone or cool-blue accent
#         preferred」に書き換え。
#     (6) compose_role_subject に palette-aware prop contrast rule を追加：
#         「Any prop held in hand must use a color from STYLE_BIBLE.color_palette
#         that visibly differs in hue family AND/OR value from the outfit's
#         dominant color. Alternate between accent (warm amber gold) and
#         main_color (muted warm blue) depending on outfit — do NOT default
#         to amber gold for every prop」。STYLE_BIBLE 整合かつ amber gold
#         収束を予防。本ルールは role-based subject_block にのみ適用、
#         nationality 系には適用しない（subject_block_pattern が異なる）。
#     (7) PART 2.2 ROLE_BASED_GENERIC_PROFILES の note に prop contrast rule
#         の適用範囲を明記（role only、nationality 系には適用されない）。
#
...
##
## ────────────────────────────────────────────────────────────
## v3.12 (2026-05-21): 3 universal rules（普遍ルール + 小さな lookup + deterministic hash）
## ────────────────────────────────────────────────────────────
##
## 背景: v3.11.1 までは PERSON_NATIONALITY_HINTS の apparent_features_hint /
##   cultural_styling_hint に enumerate 表現が並んでおり、単一画像生成では
##   Imagen / nanobanana が中央値に倒れて phenotype が画一化する事故が再発し
##   続けた（v3.8 以降の構造欠陥）。v3.12 では 3 つの問題を「per-country
##   ハードコード」ではなく「普遍ルール + 小さな lookup + deterministic
##   word hash」の同型構造で恒久解決する。過去 v3.12 ハードコード版
##   （5 commit, 14eb069 → 34bc3b0）は revert 済みで、本 v3.12 は universal
##   rule 版として書き直したもの。
##
## §1. PART 1.5 PHENOTYPE_SPECIFICATION_RULE 新設。
##   rule_a: enumerate ("Pick ONE from: A / B / C") 全面禁止。
##   rule_b: 隣接 2 段階までの range は許容（"fair to light-medium"）。
##           3 段階以上跨ぎは禁止。
##   rule_c: nationality nouns は COUNTRY_TO_PROFILE[word] →
##           PHENOTYPE_PROFILES[profile] で 1 つの concrete 記述。
##   rule_d: role nouns は ROLE_PHENOTYPE_PALETTE から
##           sha256(word + 'phenotype') mod N で deterministic 選択。
##   rule_e: パレットは v3.12 で fix。変更は major-version migration（v4.0+）扱い。
##   特例：americas_diverse profile は ROLE_PHENOTYPE_PALETTE にフォールバック
##         （多民族国家の扱い）。
##
## §2. PART 1.6 TRADITIONAL_DRESS_PATTERN_RULE 新設。
##   原則: Cultural traditional silhouette を指定する際は MUST 1 つの visible
##         textile pattern / embroidery / accent を含む。
##   注: プロンプト本文に kanji / hangul / 非ラテン文字を含めない（英語のみで
##       motif 名称）。
##   データ TRADITIONAL_DRESS_PATTERN_LOOKUP は scripts/build_prompts.py に置く。
##
## §3. PART 1.7 FLAG_PLACEMENT_RULE 新設。
##   原則: flag pin/patch の位置は universal options から word hash で
##         deterministic 選択。garment 上のみに限定（hat/bag/book を新たに
##         生やさない）。
##   選択: sha256(word + 'flag-placement') mod 4。
##   データ FLAG_PLACEMENT_OPTIONS は scripts/build_prompts.py に置く。
##
## §4. scripts/build_prompts.py 側の変更:
##   (a) GUIDE_PATH を v3.12 に切替。出力ファイル名を image_prompts_lesson01_v3_12.json に。
##   (b) PERSON_NATIONALITY_HINTS から apparent_features_hint 静的データを削除し、
##       phenotype_for(word) で動的計算（rule_c / rule_d）。
##   (c) flag_placement_for(word) / pattern_for(word) ヘルパー新設。
##   (d) _deterministic_pick(word, options, salt) ヘルパーで sha256 ベース選択を共通化。
##
## §5. drop された候補（v3.12 では対応しない）:
##   #1 N=1 確率揺れ（再生成で解消）/ #4 実害小 / #6 教育的に不要 /
##   #7 #8 #9 Imagen 4 固有（nanobanana 固定運用に切替したため）。
##
## ────────────────────────────────────────────────────────────
...
STYLE_BIBLE = {
  ...
  ## ============================================================
  ## v3.10 新規: VISUAL_CONTRAST_PRINCIPLE（普遍視覚対比規律）
  ## ============================================================
  ## v3.9 実機検証で「light-blue cardigan + light-blue marker pen」のような
  ## 「服と prop が同色相で prop が埋没する」事象が確認された。v3.9 では
  ## compose_role_subject 内に palette-aware prop contrast rule を加えたが、
  ## これは role 専用で他 vocab_type（concrete_object / adjective PAIR_CONTRAST
  ## / abstract_concept / spatial_relation / building）から見えない。
  ## v3.10 では STYLE_BIBLE 直下に普遍ルールとして昇格させ、将来の vocab_type
  ## 実装で同種「色被り埋没」事象を予防する。
  ##
  ## color_palette との関係：palette 自体は不変。本ルールは palette 内で
  ## 「どの 2 色を組み合わせるか」の選択規律。
  ## educational_symbol_colors との関係：symbolic 色は「意味」で選び、本ルールは
  ## 「視認性」で選ぶため両者直交。symbol_red は質問記号として常に red、
  ## visual_contrast_principle は symbol_red を置く背景や周辺要素の色を別系統に
  ## する判断を担う。
  ## ============================================================
  "visual_contrast_principle": {
    "principle":
      "Within any single image, the main visual elements (subject vs. "
      "background, person vs. prop, paired objects, symbol vs. surrounding) "
      "must use colors drawn from STYLE_BIBLE.color_palette that visibly "
      "DIFFER in hue family AND/OR value, so that each element is "
      "immediately legible against its neighbor. The palette itself is "
      "unchanged — this rule governs which combinations within the palette "
      "to choose.",

    "sub_rules_by_situation": {
      "person_prop_contrast":
        "A prop held in a person's hand or worn against the body MUST use "
        "a palette color whose hue family AND/OR value differs from the "
        "outfit's dominant color. Concretely (palette tokens): outfit "
        "= muted warm blue → prop = warm amber gold OR cool slate gray; "
        "outfit = cream/beige/white (e.g., white doctor's coat) → prop = "
        "cool slate gray OR muted warm blue; outfit = cool slate gray → "
        "prop = warm amber gold OR muted warm blue. NEVER default to "
        "warm amber gold for every prop — alternate between accent and "
        "main_color depending on outfit so that 12 role cards within a "
        "single lesson do not converge on the same prop color. "
        "Applied via compose_role_subject in scripts/build_prompts.py.",

      "pair_contrast_two_objects":
        "When two objects are shown in PAIR_CONTRAST layout (e.g., 大きい "
        "vs 小さい, 新しい vs 古い, 高い vs 安い), they MUST use two "
        "DIFFERENT palette tokens to read as visually separate items "
        "(e.g., left = muted warm blue object, right = warm amber gold "
        "object; OR left = cool slate gray, right = warm amber gold). "
        "Using the same color in both panels collapses the contrast that "
        "PAIR_CONTRAST is supposed to convey. "
        "Apply when scripts/build_prompts.py implements vocab_type=adjective.",

      "single_object_vs_background":
        "A single object placed on the cream off-white background MUST "
        "use a palette token (main_color = muted warm blue, accent = "
        "warm amber gold, or sub_color = cool slate gray) that contrasts "
        "with the cream background. Pure cream-on-cream or near-white "
        "objects (e.g., a white envelope) MUST have a clearly visible "
        "outline color (deep slate navy per STYLE_BIBLE.outline) AND a "
        "secondary accent element (e.g., a warm amber gold stamp on a "
        "cream envelope) to maintain legibility. "
        "Apply when scripts/build_prompts.py implements vocab_type"
        "=concrete_object.",

      "symbol_vs_surrounding":
        "Educational symbols (VISUAL_SYMBOLS / ARROW_SEMIOTICS / "
        "ABSTRACT_METAPHORS.color_adjustment) are chosen by MEANING per "
        "educational_symbol_colors (symbol_red = question/negative, "
        "symbol_green = checkmark, etc.) — this is orthogonal to "
        "contrast. When a symbolic color is placed, the surrounding "
        "elements must use palette tokens that visually separate the "
        "symbol from its frame (e.g., a symbol_red question mark on a "
        "cream background with a cool slate gray subject is fine; the "
        "same symbol_red on a warm amber gold subject would blur).",

      "two_persons_in_same_frame":
        "Reserved for future templates that show two persons in the same "
        "frame (e.g., G demonstrative_kosoado with speaker + listener). "
        "Two persons MUST be outfitted in different palette tokens to "
        "remain visually distinguishable.",
    },

    "rule_derivation_note":
      "Specific rule statements injected into prompts (e.g., the PROP "
      "CONTRAST RULE in compose_role_subject) are derived FROM this "
      "principle, not duplicated independently. When updating the prop "
      "contrast wording, update both this principle AND the derived "
      "rule string. Future vocab_type implementations should also "
      "derive their contrast rules from here rather than inventing new "
      "color logic.",

    "scope":
      "This rule is universally applied across all vocab_types and all "
      "templates. It does NOT replace educational_symbol_colors (which "
      "governs symbolic meaning) — the two systems operate on different "
      "axes (meaning vs. visibility) and are jointly enforced.",
  },
...
## v4.0 (2026-05-22) major-version pivot: 全国共通 modern daily casual wear +
##   国旗両手持ち（hand-held flag）に再設計。アジア／西洋アシンメトリ問題の
##   解消と教科書 nationality 挿絵慣用との整合のため。詳細はファイル冒頭
##   v4.0 経緯 + handoff_archive.md § 方針転換ログ 2026-05-22 参照。
##
## 適用対象:
...
##   ※ 「外国人」(generic role) は本ポリシーの対象外。flag を持たず、
...
NATIONALITY_NOUN_POLICY = {
  ...
  "principle":
    # v4.0 (2026-05-22): 全国共通 modern daily casual wear への pivot を反映。
    # 旧 v3.8〜v3.12 の「specific adult with culturally appropriate phenotype
    # and clothing」を「specific adult in MODERN daily casual wear typical of
    # the country today」に書き換え。伝統 silhouette は明示禁止。
    "The PERSON is the subject. A specific adult in MODERN daily casual "
    "wear typical of the country today is shown full-body, from the top "
    "of the head to the soles of both feet, spanning about 80% of the "
    "image HEIGHT (by height, NOT area), centered with empty side margins. "
    "Nationality is conveyed by the combination of (a) phenotype typical "
    "of the country (per PART 1.5 PHENOTYPE_SPECIFICATION_RULE), "
    "(b) subtle modern-fashion tendency (silhouette / fit / palette of "
    "the country's contemporary daily wear), and (c) a hand-held flag "
    "held in both hands in front of the chest. Pedagogical goal: a "
    "learner glancing at 7 nationality cards in a single lesson must "
    "distinguish each at a glance — this requires per-country modern-"
    "fashion-tendency variation + clear flag visibility. NO traditional, "
    "ceremonial, or folkloric silhouettes are permitted, even in "
    "modernized form (see NO_TRADITIONAL_SILHOUETTE_RULE in "
    "hard_constraints).",

  "flag_hand_held_rule":
    # v3.12 まで: small pin/patch (5-6% of frame, on chest or bag strap)、
    # 4 option deterministic-pick で position 選択。v4.0 (2026-05-22) で
    # 「hand-held flag, two hands, chest-front」設計へ pivot 移行。
    # 旧 flag_pin_rule は退役（archive/prompts/master_prompt_design_guide_v3_12.py 参照）。
    # v4.0.2 (第二次検証): 「2 本の旗竿が描かれる」「左右の手の高さが揃わない」
    # 事象が中国カード等で確認されたため、「単一 flag staff」「両手対称・同じ高さ」
    # の明示句を追加。
    "Render the flag as a small rectangular hand-held flag — a single "
    "fabric panel attached to ONE flag staff (the staff is a single "
    "thin vertical stick/pole), held in both hands in front of the "
    "chest. EXACTLY ONE flag staff per card — never depict two separate "
    "poles, two flags, or one hand on a flag while the other hand "
    "rests elsewhere; both hands MUST grip the SAME single flag staff. "
    "Both hands are at the SAME height on the staff (typical grip: one "
    "hand near the bottom of the staff, the other hand slightly above "
    "it on the same staff, OR both hands at roughly the same vertical "
    "position holding the staff symmetrically). Both elbows are bent at "
    "SIMILAR angles — the pose is left-right symmetric, not lopsided. "
    "The flag occupies about 12-15% of the image area — clearly "
    "readable but smaller than the subject's torso. The flag face is "
    "squarely toward the viewer (parallel to the picture plane). The "
    "flag is held at about chest-to-upper-waist height. The subject "
    "remains the clear main subject; the flag is a clear nationality "
    "cue, NOT the primary subject and NOT a dominant element of the "
    "composition. Flag size guidance: 12-15% (readable at a glance) is "
    "the target; below 8% (too small to read) and above 25% (dominates "
    "the composition) are both prohibited.",

  "hard_constraints": [
    # v4.0 (2026-05-22): hand-held flag への pivot を反映。
    # - 旧「DO NOT hold the flag in the hands or wave it」削除（pivot 設計と直接矛盾）
    # - 旧「DO NOT pose to present the flag toward the viewer」削除（同上）
    # - 旧「fill 10-12% (or more) of the frame」→ 25% 超え禁止に変更（12-15% は許可）
    # - NO_TRADITIONAL_SILHOUETTE_RULE を新規 universal rule として追加
    # - NATIONAL_SYMBOL_ISOLATION_RULE 内 flag 記述を hand-held 形式に書き換え
    "DO NOT make the flag 'PRIMARY', the 'sole identifier', or fill 25% "
    "or more of the frame. The flag is a clear nationality cue but the "
    "PERSON remains the subject.",
    "DO NOT print the flag across the whole shirt/clothing or any "
    "garment / accessory (see NATIONAL_SYMBOL_ISOLATION_RULE below).",
    "DO NOT render any text, letters, or numbers — not even on the flag.",
    # v4.0 新規 universal rule
    "NO_TRADITIONAL_SILHOUETTE_RULE (v4.0 新規・普遍ルール): "
    "Nationality cards MUST depict modern daily casual wear. Traditional, "
    "ceremonial, or folkloric silhouettes — including but NOT limited to: "
    "hanbok / jeogori (Korean traditional), qipao / cheongsam / Tang "
    "jacket (Chinese traditional), áo dài / áo bà ba (Vietnamese "
    "traditional), yukata / kimono / noragi / wagara-cut top (Japanese "
    "traditional), sari / shalwar / kurta / thawb / dashiki / kente / "
    "huipil / any other country's traditional or festival dress — are "
    "NEVER permitted, even in modernized form, even in 'everyday' or "
    "'modern-take' framing. The pedagogical principle: nationality is "
    "conveyed by phenotype + flag + subtle modern-fashion tendency, NOT "
    "by traditional dress display. This rule supersedes any per-country "
    "cultural_styling_hint phrasing that might be interpreted as "
    "suggesting a traditional silhouette.",
    "DO NOT use caricatured features (exaggerated facial features, "
    "stereotyped body proportions, ethnic-feature exaggeration).",
    "DO NOT use tourist-cliché props or backgrounds (panda, sombrero, "
    "sushi, tequila bottle, geisha-makeup, Fuji-san, cherry blossom "
    "festival, Olympics ceremony, etc.) as the nationality cue.",
    # v3.9 NATIONAL_SYMBOL_ISOLATION_RULE: v4.0 で core 規律として強化保持。
    # 「flag pin/patch (5-6%, on chest or bag strap)」記述のみ hand-held 形式に書き換え。
    # v4.0 では (d) を強化：hand-held flag があるので衣服は plain solid が原則。
    "NATIONAL_SYMBOL_ISOLATION_RULE (v3.9 / v4.0 修正・普遍ルール): "
    "The single hand-held flag held in both hands in front of the chest "
    "(12-15% of frame, per flag_hand_held_rule) is the ONLY permitted "
    "national-symbol occurrence in the image. The following are "
    "universally PROHIBITED across all current and future countries: "
    "(a) National-flag motifs, prints, or graphics on any garment, bag, "
    "or accessory (no flag t-shirts, no flag bandanas, no flag-print "
    "bags, no flag-pattern scarves, no flag-color shoelaces). "
    "(b) National-team / sports-federation / Olympic-team logos, crests, "
    "emblems, or jersey designs on any garment or accessory. "
    "(c) Garment color combinations that directly replicate the national "
    "flag's color sequence in flag-like proportions (e.g., a red top + "
    "yellow trousers for a red-yellow flag country, a green top + "
    "yellow accessory for a green-yellow flag country — the hand-held "
    "flag already carries this signal and duplication produces flag-like "
    "imagery on the body). "
    "(d) Garments described as 'graphic', 'printed', 'logo', 'patterned' "
    "or with unspecified print contents. Always EITHER specify a plain "
    "solid color OR name a non-national, non-team pattern explicitly "
    "(e.g., subtle stripes, dots, abstract shapes, botanical print in "
    "muted modern colors). v4.0 strengthens (d): since the hand-held "
    "flag now carries the clear nationality signal, garments SHOULD "
    "default to plain solid color or muted modern check / botanical / "
    "geometric prints — never traditional textile motifs (wagara / "
    "asanoha / seigaiha / phoenix / dragon / etc., which were permitted "
    "as cultural cues in v3.12 are now redirected by "
    "NO_TRADITIONAL_SILHOUETTE_RULE).",
  ],

  "subject_block_pattern":
    # v4.0 (2026-05-22): hand-held flag への pivot を反映。
    # 旧 v3.12:  "A small {FLAG_SHAPE_AND_COLORS} national-flag pin or cloth
    #            patch (about 5-6% of the frame) is positioned {FLAG_PLACEMENT}
    #            as a subtle nationality cue."
    # v4.0:     hand-held flag in both hands in front of chest, 12-15%, flag face
    #           squarely toward viewer.
    # {FLAG_PLACEMENT} placeholder は機構として維持（build_prompts.py の
    # flag_placement_for(word) が定数 "in both hands in front of the chest" を
    # 返すように改修）。template / preflight の breaking change を避けるため。
    # subject の body は 3/4 view と両手持ち pose の緊張関係を緩和するため
    # 「15-30 degrees off frontal」を許容する明示句を inline 追加。
    "A specific adult person. {APPARENT_FEATURES_HINT} {CULTURAL_STYLING_HINT} "
    "Neutral and welcoming expression. The subject is holding a small "
    "rectangular {FLAG_SHAPE_AND_COLORS} hand-held flag attached to a "
    "SINGLE flag staff (one thin vertical pole only — NEVER two separate "
    "poles), {FLAG_PLACEMENT}, flag face squarely toward the viewer. Both "
    "hands grip the SAME single staff at similar heights with elbows "
    "bent at SIMILAR symmetric angles — never one hand high and the "
    "other low. The flag occupies about 12-15% of the image area as a "
    "clear nationality cue. The person remains the clear main subject; "
    "the flag is a clear identifier but NOT the primary subject. The "
    "subject's body MAY face the viewer slightly more head-on (about "
    "15-30 degrees off frontal, instead of the standard 30-45 degree "
    "3/4 view) to accommodate the chest-front flag pose, while still "
    "preserving natural body proportions and standing posture. "
  ...
  "cultural_styling_hint_definition":
    # v4.0 (2026-05-22): modern wear のみへの統一を反映。
    # 旧 v3.8: 3 ルート許容 (a) contemporary fashion (b) modernized cultural dress
    #          (c) everyday traditional dress
    # v4.0:    modern daily casual wear のみ。silhouette/fit/palette tendency で
    #          国別 subtle 弁別。伝統 silhouette は NO_TRADITIONAL_SILHOUETTE_RULE で禁止。
    "{CULTURAL_STYLING_HINT} = a description of modern daily casual wear "
    "typical of the country today, expressed in concrete contemporary "
    "garment-type terms (silhouette, fit, layering, footwear, optional "
    "non-hand accessory). All 7 countries use modern wear only — "
    "traditional / modernized-traditional / ceremonial silhouettes are "
    "NEVER permitted (per NO_TRADITIONAL_SILHOUETTE_RULE in "
    "hard_constraints). Per-country variation is achieved by subtle "
    "differences in (i) silhouette / fit (e.g., minimalist contemporary "
    "vs. clean tailored vs. relaxed casual), (ii) palette tendency "
    "(country-today's modern-fashion muted color preference — NEVER "
    "flag-color combinations), (iii) footwear choice (sneakers / loafers / "
    "espadrille-style flats / etc.), and (iv) optional non-hand "
    "accessory (shoulder-strap bag / scarf / cap / wristwatch — but "
    "NEVER held in hand, since the hand-held flag occupies both hands). "
    "Caricature / folkloric national-costume display / tourist cliché "
    "remain prohibited (see hard_constraints).",

  "apparent_features_hint_definition":
    # v3.12 (= v4.0 維持): PHENOTYPE_SPECIFICATION_RULE (PART 1.5) 経由で
    # phenotype_for(word) が単一 concrete 記述を供給。
    # v4.0 では PART 1.5 rule_e palette_freeze を継続：major-version migration
    # を機会としつつ phenotype 変更の必然性が無いため PHENOTYPE_PROFILES /
    # ROLE_PHENOTYPE_PALETTE / PHENOTYPE_SALT を変更しない。
    "{APPARENT_FEATURES_HINT} = a SINGLE concrete description of the person's "
    "phenotype (skin tone, hair color, hair texture) typical and culturally "
    "appropriate for the country today, written per PHENOTYPE_SPECIFICATION_"
    "RULE (PART 1.5): one concrete value per dimension (rule_a), with "
    "optional adjacent 2-step ranges (rule_b). v3.8〜v3.11.1 で使われていた "
    "enumerate 形式 ('Pick ONE from: A / B / C') は PART 1.5 rule_a により "
    "v3.12 で禁止（v4.0 でも維持）。 "
    "Diversity is achieved across cards via deterministic per-word selection "
    "in scripts/build_prompts.py phenotype_for(word), NOT within a single "
    "card's prompt. Nationality nouns route via COUNTRY_TO_PROFILE → "
    "PHENOTYPE_PROFILES (rule_c); the 'americas_diverse' profile falls back "
    "to ROLE_PHENOTYPE_PALETTE hash selection (rule_d).",

  "note":
    "v3.3 で multicultural variation 採用 → v3.7 で nationality 系に拡張 → "
    "v3.10 でアジア 4 か国に TWO-COLOR RULE 導入（asymmetric）→ v3.12 で 3 "
    "universal rules（PHENOTYPE / TRADITIONAL_DRESS_PATTERN / FLAG_PLACEMENT）"
    "追加 → v4.0 (2026-05-22) で全国共通 modern wear + hand-held flag に "
    "pivot 確定。TWO-COLOR asymmetric / PART 1.6 / PART 1.7 deterministic-"
    "pick は伝統 silhouette が消えたため原理的に不要化し、PART 1.6 は退役、"
    "PART 1.7 は簡素化（全員固定 pose）、TWO-COLOR asymmetric rationale は "
    "本ファイルから削除。「modern fashion tendency by country」+「hand-held "
    "flag」の組合せで弁別性を担保する設計に再構築。",
}
...
## ============================================================
## PART 1.3: ROLE_VISUAL_IDENTITY_RULE（役割視覚アイデンティティの普遍規律）
## ============================================================
## v3.10 新規。v3.9 実機検証で teacher / foreigner の visual identity が
## doctor / company_employee / student に比べて弱く、ロール即読み取り不能で
## あることが露呈。原因は (a) outfit_hints の "may carry" 表現が Imagen に
## prop 省略を許していたこと、(b) v3.7 ISOLATION_SAFE_PROPS_RULE で teacher
## の最強 signature （pointer）を削除した後、代替の必須 prop を立てていな
## かったこと。teacher 固有ではなく将来新 role 追加（警察官・店員・運転手
## 等）でも再発する構造的問題のため、普遍ルールとして明文化する。
##
## 適用：
##   - ROLE_BASED_GENERIC_PROFILES の全エントリ。
##   - 将来新 role を追加するときも本ルール準拠で outfit_hints を書く。
##   - NAMED_CHARACTER_PROFILES は character_visual_hints 側で同等の signature
##     を確保していれば本ルール準拠とみなす（既存キャラは概ね準拠済）。
##
## ISOLATION_SAFE_PROPS_RULE との関係：
##   本ルールが「mandatory prop を必須化」する範囲は、ISOLATION_SAFE_PROPS_RULE
##   が許容する isolation-safe props 内に限る。pointer / otoscope / microphone
##   等の context-dependent props は引き続き禁止。
##
## NATIONAL_SYMBOL_ISOLATION_RULE との関係：
##   garment signature や mandatory prop に national-flag motif / 国家エンブレム /
##   チームクレストを含めることは引き続き禁止（role は nationality と直交）。
##
ROLE_VISUAL_IDENTITY_RULE = {
  "principle":
    "Each role in ROLE_BASED_GENERIC_PROFILES must be identifiable as that "
    "role at a single glance, from the outfit and props alone, without scene "
    "context (since SCENE forbids non-subject objects). This requires every "
    "role to have at least one of the following visual signature types, and "
    "preferably both: (a) a garment-level signature that is essentially "
    "unique to the role (white doctor's coat, business suit, school uniform), "
    "OR (b) a mandatory prop signature drawn from ISOLATION_SAFE_PROPS_RULE "
    "(stethoscope around the neck, briefcase, backpack, lanyard name badge, "
    "visibly thick textbook). Roles whose garments alone are generic "
    "smart-casual (teacher, foreigner, future additions) MUST compensate with "
    "a mandatory prop signature.",

  "must_carry_language_rule":
    "Within ROLE_BASED_GENERIC_PROFILES.outfit_hints, the phrase 'may carry' "
    "is FORBIDDEN — Imagen interprets 'may' as optional and frequently "
    "omits the prop, removing the visual signature. Use 'MUST visibly "
    "carry' or 'MUST visibly wear' so the prop becomes a required element. "
    "Optional accessories (e.g., a wristwatch on a suit-wearing role) MAY "
    "still be written with permissive language, but the role's SIGNATURE "
    "prop (the one carrying visual identity) must be mandatory.",

  "signature_uniqueness_rule":
    "Different roles should not share their mandatory signature. If two "
    "roles converge on the same signature (e.g., both teacher and foreigner "
    "as 'generic smart-casual + textbook'), one must be differentiated by "
    "an additional signature element (e.g., teacher's lanyard name badge vs "
    "foreigner's phrasebook + day bag, or doctor's stethoscope vs nurse's "
    "scrubs + clipboard). The pedagogical test: showing a learner all "
    "current role cards in lesson_NN, each role must be unambiguously "
    "distinguishable.",

  "signature_examples_by_role_type": {
    "_purpose":
      "参考例。本辞書は build_prompts.py から参照されない。ROLE_BASED_GENERIC_PROFILES "
      "の outfit_hints を書く / audit するときの設計指針として使う。",
    "doctor":           "garment: 白衣 (open white coat) / mandatory prop: stethoscope around neck + clipboard or name badge",
    "company_employee": "garment: navy/charcoal business suit + necktie / mandatory prop: briefcase or laptop bag",
    "student":          "garment: casual smart / mandatory prop: backpack worn + visible textbook or notebook in hand",
    "teacher":          "garment: blouse + cardigan or jacket / mandatory prop: lanyard name badge (no text, small icon) + thick textbook with visible spine held in hand or under arm",
    "foreigner":        "garment: casual or smart casual / mandatory prop: language phrasebook with cover facing viewer + small day bag or shoulder strap",
    "_future_examples": {
      "police_officer":     "garment: uniform (peaked cap, badge silhouette, dark uniform shirt) — garment alone is signature",
      "restaurant_staff":   "garment: apron over plain shirt / mandatory prop: order pad or small tray held in hand",
      "delivery_driver":    "garment: vest or jacket with reflective strip silhouette / mandatory prop: parcel held in arms",
      "chef":               "garment: white chef coat with double-breasted button row + white toque hat — garment alone is signature",
    },
  },
  ...
  "enforcement":
    ...
    "(2) 最後の要素は mandatory prop signature を 'MUST visibly carry/wear ...' "
    "形式で記述、(3) 中間に補助要素（色・任意アクセサリ）を置く。 "
    "build_prompts.py の compose_role_subject は outfit_hints をそのまま "
    "Outfit and props 行に展開するため、本ルール準拠の文言がそのまま Imagen に "
    "渡る。pre-flight invariant としての mechanical check は v3.10 段階では "
    "実装しない（'may carry' 文字列の grep 程度の軽い check は将来追加可能）。"
    "v3.11: must_carry_language_rule は PART 1.4 PROMPT_LITERALIZATION_"
    "AVOIDANCE_RULE に普遍化された（role 限定ではなく全プロンプト記述に適用）。",
}


## ============================================================
## PART 1.4: PROMPT_LITERALIZATION_AVOIDANCE_RULE（プロンプト literalize 回避規律）
## ============================================================
## v3.11 新規。v3.10 実機検証で teacher.outfit_hints に書いた
## "a small generic icon such as a book or pencil silhouette"（汎用 ID badge
## を意図した記述）を Imagen が「draw a book or pencil silhouette」と
## literalize し、首から鉛筆マーク付きペンダントが下がる絵を出した。
##
## これは teacher 単独の問題ではなく、ガイド全体に潜在する汎用パターン：
##   - "such as / e.g. / for example / including X" を「example として」書くと、
##     Imagen はその example をそのまま draw instruction として literalize する
##   - "may / might / optional / possibly" を書くと、Imagen は省略可能と解釈し、
##     しばしば prop / detail を omit する（v3.10 ROLE_VISUAL_IDENTITY_RULE.
##     must_carry_language_rule で role 限定で規律化済 → 本ルールで普遍化）
##   - "a small icon" / "some kind of decoration" 等の vague descriptor は
##     Imagen が自由に補完し、意図しない要素が描画される
##
## 適用：
##   - 本ガイド全体のプロンプト記述（STYLE_BIBLE 含む全 PART）に適用
##   - scripts/build_prompts.py 内の compose_* 関数の return 文字列も含む
##   - PERSON_NATIONALITY_HINTS / ROLE_BASED_GENERIC_PROFILES /
##     NAMED_CHARACTER_PROFILES / CHARACTER_PROFILES の全テキスト field に適用
##
## 注：本ルールは「ガイド執筆者向けの規律」であり、Imagen に渡るプロンプト
## 文字列そのものではない。ガイド執筆/audit 時に本ルール準拠で書く。
##
PROMPT_LITERALIZATION_AVOIDANCE_RULE = {
  "principle":
    "Imagen treats every concrete noun, color, shape, or pattern in the "
    "prompt as a draw instruction. Words intended as 'examples' or "
    "'placeholders' (such as / e.g. / for example / including) are NOT "
    "interpreted as illustrative — they become literal draw targets. "
    "Similarly, modal verbs (may / might / optional / possibly) signal "
    "to Imagen that the element is omittable, and it is often omitted. "
    "Author every line of every prompt with this literalization in mind: "
    "say exactly what you want drawn, and nothing more.",

  "rule_a_no_examples_for_placeholders":
    "When the intent is a BLANK or PLACEHOLDER element (e.g., a generic "
    "ID badge with no specific design, a plain envelope, an abstract "
    "background shape), NEVER write 'such as X or Y' or 'e.g., A, B, or C' "
    "to suggest possible content — Imagen will draw X, Y, A, B, or C "
    "literally. Instead write 'blank', 'plain', 'no marking', 'no icon', "
    "'no decoration', 'no graphic', or 'unspecified content with no "
    "drawn elements' to force the absence. "
    "BAD: 'a small icon such as a book or pencil silhouette' → Imagen "
    "draws a pencil. "
    "GOOD: 'a plain blank rectangle with NO icon, NO text, NO graphic' → "
    "Imagen draws an empty rectangle.",

  "rule_b_examples_only_when_literal":
    "When 'such as / e.g.' IS the intended use (i.e., providing concrete "
    "draw choices that Imagen should literally pick from), this is "
    "ACCEPTABLE — but make the 'pick ONE' nature explicit. "
    "OK: 'Pick ONE wagara pattern: asanoha (hemp-leaf), seigaiha (wave), "
    "or shippō (overlapping circles)' — Imagen picks one of the three "
    "and draws it, which is the desired behavior. "
    "OK: 'a botanical print such as palm leaves, monstera, or hibiscus "
    "in earthy tones' — any of these are acceptable to draw. "
    "The distinguishing test: do you want Imagen to literally draw the "
    "example? If yes → fine. If no → use rule_a.",

  "rule_c_must_or_never_not_may":
    "Modal verbs 'may / might / optional / possibly' are NOT useful in "
    "Imagen prompts — they signal omittability and are often acted on. "
    "Universally rewrite: "
    "(1) Elements that should appear → 'MUST visibly carry/wear/include' "
    "(2) Elements that should NOT appear → 'MUST NEVER include' / "
    "'absolutely no' / 'is PROHIBITED' "
    "(3) Genuine optional accessories (where presence/absence both ok) → "
    "use 'MAY be present' (uppercase MAY) sparingly, and accept either "
    "outcome. v3.10 ROLE_VISUAL_IDENTITY_RULE.must_carry_language_rule "
    "applied this to ROLE_BASED_GENERIC_PROFILES; v3.11 generalizes it.",

  "rule_d_specify_dont_describe":
    "Vague descriptors ('a small icon', 'some decoration', 'a touch of "
    "color', 'a slight accent') leave too much freedom and Imagen "
    "compensates with arbitrary content. Always specify: "
    ...
    "If genuine freedom is desired in one of these axes, set the others "
    "tightly to constrain the outcome.",

  "audit_checklist_for_authors": [
    "Search the file/section for 'such as', 'e.g.', 'for example', "
    "'including', 'like a' — for each match, verify rule_a or rule_b applies",
    "Search for 'may', 'might', 'possibly', 'optional', 'sometimes' — "
    "rewrite per rule_c (preserve genuine optionality with uppercase MAY)",
    "Search for 'small icon', 'some', 'a touch of', 'a slight', 'a hint of' "
    "— rewrite per rule_d (specify shape/size/color/content)",
    "For each BLANK/PLACEHOLDER intent, confirm the prompt says 'blank' / "
    "'no [thing]' explicitly, not 'a generic X'",
  ],

  "enforcement":
    "本ルールはガイド執筆/audit 時の人間規律。pre-flight invariant としての "
    "mechanical check は v3.11 段階では実装しない。将来 'such as' / 'may carry' "
    "等を文字列 grep する軽い check を scripts/build_prompts.py や "
    "scripts/invariants.mjs に追加可能。",
}


## ============================================================
## PART 1.5: PHENOTYPE_SPECIFICATION_RULE（人物 phenotype 記述の普遍規律）
## ============================================================
## v3.12 新規。v3.8 以降、PERSON_NATIONALITY_HINTS の apparent_features_hint /
## compose_role_subject の skin/hair 記述は "Pick ONE specific set of features
## from: (skin tone: A OR B OR C OR D) + (hair: X OR Y OR Z)" の enumerate 型
## だった。これは「画像生成器に discrete な選択を強制する」意図だったが、
## 実機検証（v3.8 / v3.9 / v3.10 / v3.11 / v3.11.1 すべて）で逆効果が確認された：
##
##   - Imagen / nanobanana ともに enumerate を「中央値で安定化」と解釈し、
##     skin tone は medium-darker brown、hair は dark straight black に毎回
##     収束する（"ethnically ambiguous medium tone" 問題）
##   - 複数人を生成しても phenotype 多様性が出ず、教育的弁別性が低下
##
## 構造的解決：単一画像 1 つには「1 つの concrete phenotype 記述」を渡す。
## 多様性は per-image 内で表現せず、複数画像にわたる per-card phenotype 選択
## （deterministic hash）で確保する。
##
## 適用：
##   - PERSON_NATIONALITY_HINTS の apparent_features_hint は静的データを廃止し、
##     scripts/build_prompts.py の phenotype_for(word) 関数で動的計算
##   - compose_role_subject の skin tone / hair color / hair texture enumerate も
##     phenotype_for(word) 経由の deterministic 単一記述に置換
##   - NAMED_CHARACTER_PROFILES / CHARACTER_PROFILES は本ルール対象外
##     （固定キャラは設計意図として個別 phenotype を持つため）
##
## PROMPT_LITERALIZATION_AVOIDANCE_RULE との関係：
##   PART 1.4 rule_a は「BLANK/PLACEHOLDER 意図のときに 'such as' を書くな」
##   というルールで、本 PART 1.5 rule_a は「phenotype 記述で 'Pick ONE from'
##   と enumerate を書くな」というルール。両者は独立だが、根底の問題（Imagen が
##   enumerate / example を中央値化 OR literalize する）は同じ。両ルールは
##   相補的に適用する。
##
PHENOTYPE_SPECIFICATION_RULE = {
  "principle":
    "A single image generation must receive a single concrete phenotype "
    "description, not a multi-option enumeration. Image generators (Imagen, "
    "nanobanana) interpret enumerate-style phrasing ('Pick ONE from: A / B / "
    "C') as a stability cue and collapse to the central/safest value, "
    "producing 'ethnically ambiguous medium tone' faces across all cards. "
    "Phenotype diversity is achieved across multiple images via per-card "
    "deterministic selection, NOT within a single image's prompt.",

  "rule_a_no_enumerate":
    "Within phenotype descriptions (skin tone, hair color, hair texture, "
    "facial features), the patterns 'Pick ONE from: A / B / C', 'A OR B OR "
    "C', 'choose one of A, B, C' are FORBIDDEN. Write exactly one concrete "
    "value per phenotype dimension. "
    "BAD: '(skin tone: fair OR light-medium OR olive OR brown OR deep brown) "
    "+ (hair color: black OR dark-brown OR brown OR blond OR red)' → "
    "Imagen collapses to medium-brown skin + dark-brown hair every time. "
    "GOOD: 'olive skin with straight black hair' → Imagen draws exactly that.",

  "rule_b_adjacent_range_ok":
    "Adjacent 2-step ranges within a phenotype dimension are PERMITTED "
    "because the generator interprets them as a narrow band (still producing "
    "discrete pixel values, not an enumeration to collapse). "
    "OK: 'fair to light-medium skin' (2 adjacent steps). "
    "OK: 'dark-brown to black hair' (2 adjacent shades). "
    "FORBIDDEN: 'fair to deep brown skin' (5+ steps spanning the whole "
    "spectrum — too wide; generator collapses to middle). "
    "FORBIDDEN: 'any natural hair color' (unbounded; generator picks dark).",

  "rule_c_nationality_via_lookup":
    "For nationality-noun person cards (vocab_type=person, word ends with "
    "'人'), phenotype is selected by COUNTRY_TO_PROFILE[word] → "
    "PHENOTYPE_PROFILES[profile], where each PHENOTYPE_PROFILES entry is "
    "exactly one concrete sentence written per rule_b. The mapping and "
    "the 7 profile sentences are encoded in scripts/build_prompts.py and "
    "may be expanded in future versions (adding a new country = adding one "
    "row to COUNTRY_TO_PROFILE; adding a new profile = adding one entry "
    "to PHENOTYPE_PROFILES). "
    "Special case: COUNTRY_TO_PROFILE may map a country to "
    "'americas_diverse', which falls back to ROLE_PHENOTYPE_PALETTE-style "
    "deterministic selection (per rule_d) because the country's population "
    "is too multi-ethnic for a single concrete description to be "
    "representative.",

  "rule_d_role_via_hash":
    "For role-noun person cards (vocab_type=person, word does NOT end with "
    "'人' — e.g. 医者 / 学生 / 先生 / 外国人 / 会社員), and for nationality "
    "nouns mapped to 'americas_diverse' (rule_c special case), phenotype is "
    "selected from ROLE_PHENOTYPE_PALETTE (6 entries spanning fair → deep "
    "brown with varied hair) via deterministic word hash: "
    "sha256(word + SALT) mod 6, where SALT is a short string chosen so "
    "that the lesson's role+americas word set distributes evenly across "
    "the 6 buckets (in v3.12, build_prompts.py uses SALT='palette-pick', "
    "selected empirically for lesson_01 to achieve skew=1, the theoretical "
    "minimum). The same word always yields the same phenotype across "
    "regenerations (reproducibility), but 5-6 role cards within a single "
    "lesson visibly differ from each other (per-lesson diversity). "
    "Salt is frozen at v3.12 (changing it shifts hash output and breaks "
    "reproducibility — treat as major-version migration per rule_e).",

  "rule_e_palette_freeze":
    "PHENOTYPE_PROFILES (7 entries) and ROLE_PHENOTYPE_PALETTE (6 entries) "
    "are FROZEN at v3.12. Changing either palette modifies the deterministic "
    "hash output for the same word and breaks reproducibility of "
    "previously-generated images. Palette modifications are treated as a "
    "major-version migration (v4.0+) requiring full re-generation and "
    "explicit migration notes. Adding a NEW country to COUNTRY_TO_PROFILE "
    "(routing to an existing PHENOTYPE_PROFILES entry) is a minor change "
    "and does NOT require a major-version bump.",

  "data_location_note":
    "Concrete data (PHENOTYPE_PROFILES, COUNTRY_TO_PROFILE, "
    "ROLE_PHENOTYPE_PALETTE) lives in scripts/build_prompts.py, NOT in this "
    "guide. The guide states only the universal RULE. This separation lets "
    "future lesson designers add countries/roles without modifying the "
    "design guide, while keeping the rule itself version-controlled here. "
    "新国籍追加時の自動発動フローは FLAG_SHAPE_DETAIL_RULE (PART 1.9) の "
    "country_addition_checklist を参照（ABORT 検知トリガー + phenotype_profile "
    "選定基準 + flag_shape_and_colors / cultural_styling_hint 案作成 + "
    "build_prompts.py 追記手順）。本ルールの COUNTRY_TO_PROFILE 拡張は "
    "そのチェックリスト手順 (4) に統合されている。",

  "enforcement":
    "本ルールはガイド執筆/build_prompts.py 実装の人間規律。 "
    "build_prompts.py の phenotype_for(word) 関数が rule_c / rule_d を実装する。"
    "pre-flight invariant としての mechanical check（'Pick ONE from' や "
    "' OR ' を含む phenotype 文字列の grep）は v3.12 段階では実装しない。"
    "将来 scripts/invariants.mjs に C 検査として追加可能。",
}


## ============================================================
## PART 1.6: TRADITIONAL_DRESS_PATTERN_RULE（v4.0 で退役）
## ============================================================
## v3.12 新規 → v4.0 (2026-05-22) major-version pivot で退役。
##
## 退役理由：
##   v3.12 PART 1.6 は「アジア 4 か国の cultural traditional silhouette に
##   1 つの visible textile pattern を MUST 含める」universal rule だった。
##   v4.0 では全国共通 modern daily casual wear に統一し、伝統 silhouette
##   自体が NO_TRADITIONAL_SILHOUETTE_RULE (PART 1.1 hard_constraints) で
##   明示禁止となったため、本ルールが適用される situation が原理的に消滅。
##
## 過去経緯（参考保全）：
##   v3.10 で TWO-COLOR RULE をアジア 4 か国に追加 → silhouette は出るが
##   pattern が無く plain solid 化 → v3.12 PART 1.6 で textile pattern
##   必須化を導入 → v4.0 で伝統 silhouette 自体を廃止し本ルールも退役。
##   旧 RULE 全文（principle / rule_a 〜 rule_e / data_location_note /
##   enforcement）は archive/prompts/master_prompt_design_guide_v3_12.py
##   line 1684-1778 を参照。
##
## v4.0 における代替方針：
##   伝統 silhouette が消えたため textile pattern の必須化問題は発生しない。
##   modern garment の subtle pattern（plaid check / botanical / geometric
##   in muted modern colors）は PERSON_NATIONALITY_HINTS の各国
##   cultural_styling_hint で per-country に specify する。pattern lookup
##   はもはや不要（build_prompts.py の TRADITIONAL_DRESS_PATTERN_LOOKUP /
##   pattern_for(word) も v4.0 で削除）。
##
## 将来の再活性化条件：
##   将来 v5.0+ で「伝統服を含む文化的多様性を modern wear と並列で扱う」
##   方針に転換した場合、本ルールを restore する設計判断が再び発生し得る。
##   ただしその際は「アジア / 西洋アシンメトリ」を再発させない構造が必要。
##


## ============================================================
## PART 1.7: FLAG_PLACEMENT_RULE（v4.0 で簡素化・全員固定 pose）
## ============================================================
## v3.12 新規 → v4.0 (2026-05-22) major-version pivot で簡素化。
##
## 簡素化理由：
##   v3.12 PART 1.7 は flag pin/patch の位置を 4 option（左胸 / 右胸 /
##   左袖 / 右袖）から word hash で deterministic に選択する rule だった。
##   v4.0 では flag が「両手で胸前に持つ hand-held flag」固定 pose に
##   なったため、position の選択肢が原理的に 1 つに収束。deterministic-
##   pick 機構は不要となり、{FLAG_PLACEMENT} placeholder には全員同じ
##   定数文字列（"in both hands in front of the chest"）が供給される。
##
## 過去経緯（参考保全）：
##   v3.12 では 4 option × sha256(word+'flag-placement') mod 4 で
##   deterministic 選択。lesson_01 の 7 nationality word が 4 bucket に
##   どう分散するかを hash_distribution_check で検証する設計だった。
##   旧 RULE 全文（principle / rule_a 〜 rule_d / data_location_note /
##   hash_distribution_check / enforcement）は archive/prompts/
##   master_prompt_design_guide_v3_12.py line 1781-1858 を参照。
##
## v4.0 における規律（簡素化版）：
##
FLAG_PLACEMENT_RULE = {
  "principle":
    "v4.0 (2026-05-22): all nationality cards use a fixed flag pose — "
    "the subject holds a small rectangular hand-held flag in both hands "
    "in front of the chest, flag face squarely toward the viewer. There "
    "is no per-card placement variation. The {FLAG_PLACEMENT} "
    "placeholder in NATIONALITY_NOUN_POLICY.subject_block_pattern is "
    "kept for backward compatibility with the v3.12 pipeline structure "
    "but is now filled by a constant phrase rather than a deterministic "
    "pick.",

  "rule_a_constant_placement":
    "{FLAG_PLACEMENT} = 'in both hands in front of the chest, at about "
    "chest-to-upper-waist height, both arms bent at the elbows' — the "
    "same string for all 7 nationality words in lesson_01 and for any "
    "future country added. The flag pose is part of the v4.0 universal "
    "design (not per-country variation).",

  "rule_b_size_governed_by_flag_hand_held_rule":
    "Flag size (12-15% of frame, per PART 1.1 flag_hand_held_rule) is "
    "governed by NATIONALITY_NOUN_POLICY.flag_hand_held_rule, not by "
    "this rule. This rule governs placement (= constant 'in both hands "
    "in front of the chest') only.",

  "rule_c_single_occurrence_preserved":
    "Exactly ONE flag occurrence per card (per NATIONAL_SYMBOL_"
    "ISOLATION_RULE). v3.12 from rule_c is preserved unchanged.",

  "data_location_note":
    "FLAG_PLACEMENT_OPTIONS in scripts/build_prompts.py is now a single-"
    "entry list (or a constant string) — kept as a list for backward-"
    "compatible API signature with v3.12's _deterministic_pick helper. "
    "flag_placement_for(word) returns the single constant string "
    "regardless of word. No per-word hashing, no salt freezing, no "
    "skew analysis needed (since all words map to the same string).",

  "enforcement":
    "build_prompts.py の flag_placement_for(word) ヘルパーが constant "
    "string を返し、NATIONALITY_NOUN_POLICY.subject_block_pattern の "
    "{FLAG_PLACEMENT} placeholder に注入する。pre-flight invariant としての "
    "mechanical check は v4.0 段階では実装しない（v3.12 と同じ）。",
}


## ============================================================
## PART 1.8: FACIAL_FEATURES_RULE（顔のディテール必須化規律）
## ============================================================
## v4.0 (2026-05-22) 新規。v4.0 実機検証（lesson_01 全 12 件 nanobanana 生成）で
## 会社員カード 1 件において顔のディテール（目鼻口眉）が完全欠落する事象が確認
## された。他 11 件は顔が描画されており、確率揺れの可能性が高いが、特定の構図
## （suit + briefcase + 厳格姿勢）が「faceless silhouette stock image」の bias を
## 強める潜在的構造問題と判断。将来 role 追加（警察官・店員・銀行員等の uniform
## ベース role）でも同種事象が再発するリスクがあるため、universal rule として
## 明文化する。
##
## 適用：
##   - PROMPT_TEMPLATES["vocabulary_person"] 経由の全 person カード
##     （role / nationality / NAMED_CHARACTER すべて）。
##   - vocab_type=person 以外（building / object 等）には適用しない。
##
## NATIONAL_SYMBOL_ISOLATION_RULE / NO_TRADITIONAL_SILHOUETTE_RULE との関係：
##   両ルールは「身体に何を描かないか」を規定する。本ルールは「顔に何を描く
##   ことを必須化するか」を規定し、両者は直交。
##
FACIAL_FEATURES_RULE = {
  "principle":
    "All person vocabulary cards MUST clearly depict all four primary "
    "facial features (eyes, eyebrows, nose, mouth) drawn in the same "
    "flat illustration style as the rest of the body. Faceless "
    "silhouettes, blank faces, partially-rendered faces, or 'stylized "
    "illustration without facial detail' are NEVER permitted. The "
    "pedagogical principle: a vocabulary card is a character a learner "
    "looks at and identifies with — a faceless figure breaks that "
    "identification and reads as anonymous stock imagery, not a "
    "specific role/nationality.",

  "rule_a_four_required_features":
    ...
    "(3) nose (a simple line, dot, or small shape — does not need to be "
    "anatomically detailed), "
    "(4) mouth (a simple line, curve, or small shape — closed mouth in "
    "a calm expression is fine; teeth not required). "
    "Omitting any one of these four counts as a faceless violation.",

  "rule_b_consistent_with_flat_style":
    "Facial features are drawn in the same flat illustration style as "
    "the rest of the body — clean continuous outlines, no gradients, "
    "no photoreal shading, no anatomical detail beyond what the flat "
    "vector style supports. The intent is 'minimal but present', not "
    "'detailed portrait'. Even minimal features (a dot for the eye, a "
    "short curve for the mouth) satisfy the rule as long as they are "
    "visibly drawn.",

  "rule_c_avoid_omission_trigger_contexts":
    "nanobanana tends to omit facial features more often when the "
    "prompt heavily emphasizes a non-face signature (e.g., "
    "'business suit + briefcase + professional posture' biases toward "
    "faceless stock illustration aesthetic). For these high-risk role "
    "compositions (company employee, doctor in full coat, banker, "
    "police officer, chef in toque), the rule MUST be doubly enforced "
    "by inline directive in PROMPT_TEMPLATES['vocabulary_person'] (see "
    "enforcement below). Do NOT rely on the default behavior; always "
    "include the FACIAL FEATURES directive.",

  "rule_d_named_character_consistency":
    "NAMED_CHARACTER_PROFILES (鈴木さん等 固定キャラ) already specify "
    "facial features via fixed_features.face. Those entries already "
    "satisfy this rule. New named-character additions MUST include a "
    "fixed_features.face entry per this rule.",

  "enforcement":
    "PROMPT_TEMPLATES['vocabulary_person'] の [SUBJECT] セクション末尾に "
    "FACIAL FEATURES inline directive を埋め込む（v4.0 で追加）。"
    "directive 文字列は本ルール rule_a / rule_b 準拠で書き、全 person "
    "プロンプト（role / nationality / named-character）に自動的に "
    "適用される。pre-flight invariant としての mechanical check は v4.0 "
    "段階では実装しない（'face|facial' grep で軽い check を将来追加可）。",
}


## ============================================================
## PART 1.9: FLAG_SHAPE_DETAIL_RULE（国旗形状記述の詳細度規律）
## ============================================================
## v4.0 (2026-05-22) 新規。v4.0 実機検証（lesson_01 全 7 国籍カード nanobanana
## 生成）で、シンプルな国旗（日 = 白＋赤丸、韓 = 太極＋四卦、ベトナム = 赤＋黄星）
## はほぼ正確に再現されたが、複雑な国旗（米 = 50 stars / 13 stripes、西 = 紋章 +
## 1:2:1 stripe、伯 = 天球 + 曲線 band）は構造要素の欠落・簡略化過多が確認された。
## flag は国家アイデンティティの中核であり、教材として正確性を可能な限り担保
## すべき。サイズ的に 100% pixel-level 再現は無理だが、特徴的構造要素を string
## レベルで明示することで再現性が大きく向上する。
##
## 適用：
##   - PERSON_NATIONALITY_HINTS[国名].flag_shape_and_colors の記述ガイドライン。
##   - NATIONALITY_NOUN_POLICY.flag_shape_and_colors_hint の補強。
##   - 新国籍追加時の品質基準として機能（後述チェックリスト参照）。
##
## NO_TEXT_RULE / NATIONAL_SYMBOL_ISOLATION_RULE との関係：
##   実際の国旗にテキストがある場合（伯 "Ordem e Progresso"、巴西の州章テキスト
##   等）でも、flag 上のテキストは描かない（NO_TEXT_RULE の絶対規律）。テキスト
##   要素は silhouette として記述する（"curved white band — no text on the band"）。
##
FLAG_SHAPE_DETAIL_RULE = {
  "principle":
    "Each country's flag_shape_and_colors string MUST follow a three-"
    "layer template: (1) INVOKE the flag by its formal name to "
    "leverage nanobanana's learned visual knowledge of standard "
    "world flags, (2) provide a MINIMAL STRUCTURAL ANCHOR describing "
    "the most distinguishing visible elements (band orientation, "
    "color sequence, central or corner emblem position), and "
    "(3) ENFORCE NO_TEXT_RULE explicitly. This three-layer approach "
    "outperforms exhaustive geometric specification because nanobanana "
    "is an LLM-based generator with substantial learned knowledge of "
    "fixed world symbols like national flags — over-specifying every "
    "stripe count, star count, or shield internal heraldry introduces "
    "noise that conflicts with the learned representation.",

  "rule_a_invoke_by_formal_name":
    "Begin the flag_shape_and_colors string with 'the actual flag of "
    "<country>' or 'the actual <country> flag' — where the flag has "
    "a widely-recognized formal name in common English-language usage, "
    "append it in parentheses: 'flag of Japan (Hinomaru)', 'flag of "
    "South Korea (Taegukgi)', 'flag of the United States (Stars and "
    "Stripes)'. The word 'actual' signals to nanobanana that this is "
    "a real-world symbol it should render from its learned "
    "representation, NOT a freely-synthesized abstract design. "
    "Transliterations only — see rule_e.",

  "rule_b_minimal_structural_anchor":
    "Follow the invoke phrase with 1-2 short sentences describing the "
    "most distinguishing visible structure: band orientation "
    "(horizontal/vertical), color sequence, central or corner emblem "
    "presence and approximate position. Do NOT exhaustively enumerate "
    "stripe counts, star counts, or exact proportions unless these are "
    "particularly load-bearing for the flag's identity (e.g., Spain's "
    "1:2:1 band proportion is load-bearing because flags with red and "
    "yellow bands of equal width read as different flags). nanobanana "
    "cannot count to 50 stars perfectly even with specification — "
    "trust the invoke phrase for fine details and let the learned "
    "representation handle them.",

  "rule_c_enforce_no_text":
    "End every flag_shape_and_colors string with an explicit NO_TEXT "
    "enforcement clause: 'ABSOLUTELY no text, letters, numbers, or "
    "words anywhere on the flag'. For flags whose actual real-world "
    "version contains text (Brazilian 'Ordem e Progresso' band, Saudi "
    "shahada inscription, Iraqi takbir, etc.), append a targeted "
    "override: 'even if the actual flag contains text/words in that "
    "area, render that area as blank/textless'. This overrides "
    "nanobanana's learned knowledge specifically for the text-"
    "rendering case while leaving the rest of the learned "
    "representation intact.",

  "rule_d_size_realism":
    "At 12-15% of frame size (per flag_hand_held_rule), the rendered "
    "flag is roughly 150-200 pixels per side. Exact pixel-level "
    "reproduction is impossible. The invoke phrase + minimal anchor + "
    "NO_TEXT enforcement typically achieves 'recognizable at a glance' "
    "fidelity at the correct level. Adding more geometric detail beyond "
    "the minimal anchor often introduces noise (rule_b).",

  "rule_e_latin_script_only_in_prompts":
    "Even when invoking by formal name, use Latin-script "
    "transliteration only (Hinomaru, Taegukgi, Bandera de España, "
    "Stars and Stripes). NEVER include kanji, hangul, Chinese "
    "characters, Arabic script, Cyrillic, etc. in the prompt itself "
    "— nanobanana may attempt to render the non-Latin characters as "
    "text on the flag, violating rule_c.",

  "rule_f_invoke_priority_over_geometric_specification":
    "v4.0.3 design decision (2026-05-22): when invoke-by-name and "
    "geometric specification conflict in writing strategy, INVOKE "
    "WINS. Historical example: v4.0.1 / v4.0.2 specified for Spain "
    "'a small heraldic shield-shape (an escutcheon: a flat horizontal "
    "top edge, straight vertical sides for the upper portion, and a "
    "rounded curved point at the bottom)' — this exhaustive geometric "
    "description was nanobanana-suboptimal because nanobanana already "
    "knows the actual Spanish coat-of-arms silhouette from training. "
    "The v4.0.3 form 'the actual flag of Spain (Bandera de España)... "
    "with the Spanish coat-of-arms positioned on the yellow band "
    "toward the left side' invokes the learned representation cleanly "
    "and let nanobanana handle the heraldry details.",

  "data_location_note":
    "Per-country strings live in scripts/build_prompts.py "
    "PERSON_NATIONALITY_HINTS[country].flag_shape_and_colors. The "
    "guide states the rule; the lookup is data that future country "
    "additions extend. 新国籍追加時は本ルール準拠で string を作成 → "
    "lookup に追加 → 再生成で自動適用される。",

  "country_addition_checklist":
    # ★ Claude Code 自動発動チェックリスト ★
    # 本リストは Claude Code が build_prompts.py の ABORT 出力を検知した
    # 時点で自動的に発動する想定。ユーザーからの明示依頼を待たない。
    # FACIAL_FEATURES_RULE / NO_TRADITIONAL_SILHOUETTE_RULE / PHENOTYPE_
    # SPECIFICATION_RULE と相互参照しながら新国籍 entry を整備する。
    "新国籍追加チェックリスト（Claude Code 用・自動発動）: \n"
    "\n"
    "  【発動条件】\n"
    "    scripts/build_prompts.py が 'ABORT: N 件の person が役割/国籍に\n"
    "    分類できない' を出力したとき、unclassified word が '〜人' で\n"
    "    終わる場合。ユーザーから明示依頼を待たず、Claude Code が ABORT\n"
    "    出力を見た時点で本チェックリストを自動発動する。\n"
    "\n"
    "  【手順】\n"
    "    (1) ABORT 出力から unclassified word を抽出。\n"
    "    (2) flag_shape_and_colors を本ルール 3-layer template に従って\n"
    "        作成案を作成:\n"
    "          Layer 1 (rule_a INVOKE): 'the actual flag of <country>'\n"
    "                  + formal name in parentheses if widely-known.\n"
    "          Layer 2 (rule_b ANCHOR): 1-2 sentences on band orientation\n"
    "                  / color sequence / central or corner emblem position.\n"
    "                  Avoid exhaustive enumeration.\n"
    "          Layer 3 (rule_c NO_TEXT): 'ABSOLUTELY no text, letters,\n"
    "                  numbers, or words anywhere on the flag' + targeted\n"
    "                  override if the actual flag contains text.\n"
    "        Latin-script only (rule_e). Trust nanobanana's learned\n"
    "        knowledge for fine details (rule_f).\n"
    "    (3) cultural_styling_hint を NO_TRADITIONAL_SILHOUETTE_RULE と\n"
    "        cultural_styling_hint_definition 準拠で作成案を作成\n"
    "        （modern daily casual wear / silhouette + fit + palette +\n"
    "        footwear + optional non-hand accessory / 伝統 silhouette\n"
    "        は全国禁止 / PART 1.4 rule_c 準拠で MUST / Pick ONE from\n"
    "        構文を使用）。\n"
    ...
    "          既存 profile で表現できない場合のみ PHENOTYPE_PROFILES に\n"
    "          新 profile 追加を提案（palette_freeze 例外なので慎重に、\n"
    "          PART 1.5 rule_e 参照、user 合意必須）。\n"
    "    (5) ユーザーに案を提示して合意を取る（CLAUDE.md「教育内容の\n"
    "        確定は人間報告」規律に準拠）。\n"
    "    (6) build_prompts.py に追記:\n"
    "          - PERSON_NATIONALITY_HINTS['新国名'] = {...}\n"
    "          - COUNTRY_TO_PROFILE['新国名'] = '<profile_key>'\n"
    "    (7) python scripts/build_prompts.py --lesson NN で再実行 → \n"
    "        ABORT が消えることと invariants PASS を確認。\n"
    "    (8) （任意・user 判断）lesson_NN の新国籍カードを 1 枚 smoke\n"
    "        生成して視覚検証。",

  "enforcement":
    "本ルールは PERSON_NATIONALITY_HINTS[国名].flag_shape_and_colors の "
    "記述規律。build_prompts.py 側で mechanical check は v4.0 段階では "
    "実装しない。将来 invariants.mjs に「flag_shape_and_colors の長さが "
    "極端に短い（< 50 文字）国は WARN」のような軽い check を追加可能。",
}


## ============================================================
## PART 1.10: HEAD_BODY_PROPORTION_RULE（頭身比規律）
## ============================================================
## v4.0 (2026-05-22 第二次検証) 新規。v4.0 第一次実機検証（lesson_01 全 12 件
## nanobanana 生成）で会社員カードと学生カードのみ頭身が短い（5-6 頭身）
## 事象が確認された。他 10 件は adult として適切な 7 頭身。これは特定 role
## （business suit + briefcase / casual smart + backpack）が anime/cartoon
## style の child proportion bias を誘発する構造的問題と判断。
##
## 適用：
##   - PROMPT_TEMPLATES["vocabulary_person"] 経由の全 person カード。
##   - 子供を意図する NAMED_CHARACTER 例外（fixed_features.age_range に
##     "child" / "8-10 years" 等が含まれる場合）は本ルール対象外。
##   - vocab_type=person 以外には適用しない。
##
## ROLE_VISUAL_IDENTITY_RULE との関係：
##   ROLE_VISUAL_IDENTITY_RULE は「役割の即読み取り性」を担保するが、
##   role-signature が強い (suit + briefcase) ほど nanobanana が
##   child proportion bias を出しやすい。本ルールはその副作用を構造的に
##   抑える。両ルールは補完的。
##
HEAD_BODY_PROPORTION_RULE = {
  "principle":
    "All person vocabulary cards (except explicitly child NAMED_CHARACTERs) "
    "MUST depict an adult standing figure with approximately 7-head-height "
    "proportion: the total body height from top of head to bottom of feet "
    "is roughly 7 times the head height. This is the standard adult human "
    "proportion in educational illustration. nanobanana tends to default "
    "to 5-6 head proportion (childlike/cartoonish) when role signatures "
    "are dominant (business suit, school casual, uniform), so explicit "
    "directive is required.",

  "rule_a_seven_head_proportion":
    "Adult standing figure: head height = approximately 1/7 of total body "
    "height. Legs from hip to floor occupy approximately the lower half "
    "(3.5/7) of the body. Torso (shoulder to hip) occupies approximately "
    "2.5/7. Head occupies approximately 1/7 at the top. Arms hang naturally "
    "such that fingertips reach mid-thigh when relaxed. These are "
    "guideline proportions, not pixel-exact requirements — 'adult, not "
    "childlike' is the readable outcome.",

  "rule_b_no_child_proportions":
    "NEVER render adult roles (business employee, doctor, teacher, "
    "student-as-adult, foreigner, all nationality nouns) with 5-6 head "
    "proportion which reads as childlike or cartoonish. The character "
    "must read as a working-age adult (approximately 20s-50s) by body "
    "proportion alone, even before considering facial features.",

  "rule_c_role_trigger_compensation":
    "Roles with strong childhood-bias triggers — particularly "
    "(a) business suit + briefcase (anime salaryman cliché tends "
    "toward shorter proportions), "
    "(b) school-style casual + backpack + notebook (anime student "
    "cliché tends toward teen/child proportions), "
    "(c) any uniform (police, chef, school staff) — MUST receive "
    "extra-explicit adult-proportion directive. This is enforced via "
    "PROMPT_TEMPLATES['vocabulary_person'] [SUBJECT] inline directive "
    "rather than per-role outfit_hints customization.",

  "rule_d_named_character_exception":
    "NAMED_CHARACTER_PROFILES with explicit child age_range (8-10 years, "
    "etc.) follow their own fixed_features and are exempt from this "
    "rule. New child named-characters MUST specify body proportion in "
    "fixed_features (e.g., '5-6 head proportion' for ~8-10 year olds).",

  "enforcement":
    "PROMPT_TEMPLATES['vocabulary_person'] の [SUBJECT] セクションに "
    "HEAD-BODY PROPORTION inline directive を埋め込む (v4.0.2 で追加)。"
    "directive 文字列は本ルール rule_a / rule_b / rule_c 準拠で書き、"
    "全 person プロンプトに自動適用される。pre-flight mechanical check は "
    "v4.0 段階では実装しない。",
}
...
## ============================================================
##
## v3.10 audit: PART 1.3 ROLE_VISUAL_IDENTITY_RULE 準拠で全 5 role の
## outfit_hints を以下の規律で書き直し：
##   (1) 最初の要素 = garment signature（白衣・suit・cardigan 等）
##   (2) 中間要素 = 補助（色・任意アクセサリ）
##   (3) 最後の要素 = mandatory prop signature を "MUST visibly carry/wear ..."
##       形式で記述
##   "may carry" 表現は全 role から除去（Imagen が省略するため）。
##   teacher / foreigner は generic smart-casual 系で garment signature 弱い
##   ため、mandatory prop を 2 つ（lanyard name badge + textbook 等）持たせ
##   ロール識別性を補強する。
##
## v3.10 note: prop contrast rule の適用範囲について
##   STYLE_BIBLE.visual_contrast_principle（v3.10 で新設された普遍ルール）の
##   下位適用として、compose_role_subject に palette-aware prop contrast
##   rule が組み込まれている。手に持つ prop は服装支配色と異なる hue family
##   AND/OR value を STYLE_BIBLE.color_palette から選ぶ。
##   このルールは **role-based subject_block にのみ適用** され、
##   nationality 系（compose_nationality_subject 経由）には適用されない。
##   理由：nationality 系 subject_block_pattern は phenotype + cultural
##   styling + flag pin の組み合わせで構成され、prop を中心に持たないため、
##   prop contrast rule を流し込むとルールが破綻する（特に national-flag
##   pin の色まで対比対象にされてしまう）。NATIONAL_SYMBOL_ISOLATION_RULE
##   と直交する独立規律として保持する。
## ============================================================
...
ROLE_BASED_GENERIC_PROFILES = {
  ...
  "teacher": {
    "role_ja": "先生",
    "role_en": "teacher",
    # v3.10 (ROLE_VISUAL_IDENTITY_RULE):
    #   garment alone is generic smart-casual → mandatory prop 2 個で補強。
    #   pointer (v3.7 で削除済) の代替として lanyard name badge + 厚 textbook。
    # v3.11 (PROMPT_LITERALIZATION_AVOIDANCE_RULE.rule_a):
    #   v3.10 の "a small generic icon such as a book or pencil silhouette"
    #   は Imagen が "draw a pencil silhouette" と literalize し、首から
    #   鉛筆ペンダントが下がる絵に。rule_a 準拠で「blank rectangular ID
    #   badge with NO icon, NO text, NO graphic」に書き換え。rectangular
    #   形状指定で「circular medallion pendant」誤解も予防。
    "outfit_hints": [
  ...
  "company_employee": {
    ...
    # v3.10 (ROLE_VISUAL_IDENTITY_RULE):
    #   garment signature (suit + necktie) が強いので mandatory prop は briefcase 単独で十分。
    "outfit_hints": [
  ...
  "student": {
    ...
    # v3.10 (ROLE_VISUAL_IDENTITY_RULE):
    #   backpack が signature。textbook/notebook はその上に追加して学習者性を強化。
    "outfit_hints": [
  ...
  "doctor": {
    ...
    "role_en": "doctor",
    # v3.10 (ROLE_VISUAL_IDENTITY_RULE):
    #   garment signature (white coat) が極めて強い。stethoscope は必須として明示。
    "outfit_hints": [
  ...
  "foreigner": {
    ...
    # v3.10 (ROLE_VISUAL_IDENTITY_RULE):
    #   garment alone is generic casual → mandatory prop 2 個で補強。
    #   teacher と signature 重複しないように、teacher = lanyard+textbook に対し、
    #   foreigner = phrasebook (cover facing viewer = 学習者性) + crossbody day bag
    #   （travel/visitor 性）。両 role が交わらないことを signature_uniqueness で担保。
    "outfit_hints": [
